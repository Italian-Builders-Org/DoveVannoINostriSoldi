...
try {
  for(const width of [320,390,768,1280]) {
    ...
    const ready=async count=>page.waitForFunction(count=>document.querySelectorAll('[aria-label="Allegati da inviare"] li[data-ready="true"][data-error="false"]').length===count,{},count);
    const click=label=>page.click(`button[aria-label="${label}"]`);
    const complete=()=>page.waitForFunction(()=>!!document.querySelector('[data-assistant-reply] table')&&!document.querySelector('button[aria-label="Interrompi ricerca"]'));
    ...
    await click('Invia domanda');await complete();
    ...
    await page.waitForFunction(()=>!!document.querySelector('[aria-label="Conversazione di questa pagina"] [aria-expanded="true"]'));
    ...
    await click('Rigenera risposta');await complete();assert.deepEqual(requests[1].messages[0].attachments,requests[0].messages[0].attachments);
    await click('Modifica domanda');await page.$eval('[id^="edit-message-"]',el=>{el.focus();el.select();});await page.keyboard.press('Backspace');await page.type('[id^="edit-message-"]','Confronta soltanto la biblioteca nei file.');await page.click('button::-p-text(Invia modifica)');await complete();assert.deepEqual(requests[2].messages[0].attachments,requests[0].messages[0].attachments);
    ...
    await click('Nuova chat');assert.equal(await page.$$eval('[aria-label="Allegati del messaggio"]',els=>els.length),0);
    ...
    await page.type('#assistant-prompt','Non inviare file non leggibili');assert.equal(await page.$eval('button[aria-label="Invia domanda"]',el=>el.disabled),true);await click('Rimuovi corrotto.pdf');await click('Rimuovi lungo.txt');
