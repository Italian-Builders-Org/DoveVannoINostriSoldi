#!/usr/bin/env python3
"""Pinned FC80TERRVIAB 2022 release — Viabilità e territorio function, not total services."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2022-viabilita.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2022-viabilita.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "c979ff4616dae0ce9a98f83542acd879434bddd12450e71b4e568d086ed10f3d"
...
def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """FC80TERRVIAB CSV includes Privacy (unlike FC80RIFIUTI)."""
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        "schemaVersion": 1,
        "transformVersion": 1,
        "scope": "ordinary-statute-municipalities-roads-fc80-2022",
        "referenceYear": 2022,
        ...
        "source": {
            ...
            "dataset": "Comuni · Viabilità e territorio · Indicatori e determinanti 2022 (FC80TERRVIAB)",
        ...
        "methodology": {
            ...
            "coverageWarning": "RSS e Province autonome fuori perimetro; esclusi gli aggregati di Italia, regioni, aree e fasce. Esclusi 4 Comuni con SPESA_STORICA o SPESA_STORICA_PROAB vuoti nella fonte (FST presente): nessuna imputazione a zero.",
            "rankingWarning": "I livelli della fonte non costituiscono un ranking di efficienza.",
            "functionSeparationWarning": "FC80TERRVIAB 2022 è distinto da FC80TOT 2022, FC80RIFIUTI 2022 e dalle altre funzioni: nessuna somma o confronto silenzioso fra funzioni o annualità.",
        },
    }
