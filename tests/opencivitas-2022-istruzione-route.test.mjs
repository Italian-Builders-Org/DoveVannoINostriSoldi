...
const { GET } = await import("../src/app/api/spese/opencivitas-2022-istruzione/route.ts");
const { queryOpenCivitas2022Istruzione } = await import("../src/lib/opencivitas-2022-istruzione-snapshot.ts");
const base = "http://localhost/api/spese/opencivitas-2022-istruzione";

test("FC80ISTRUZ HTTP rejects ambiguous, unbounded or cross-year requests without caching errors", () => {
...
test("FC80ISTRUZ API and MCP share pagination, provenance and region alias behavior", async () => {
  const { queryPublicDataset } = await import("../src/lib/mcp/datasets.ts");
  const { datasetCatalog } = await import("../src/lib/mcp/catalog.ts");
  const descriptor = datasetCatalog.find((item) => item.id === "opencivitas_istruzione_2022");
  ...
  assert.deepEqual(body, queryOpenCivitas2022Istruzione({ region: "Lazio", limit: 2, offset: 2 }));
  assert.deepEqual(body, await queryPublicDataset({ dataset: "opencivitas_istruzione_2022", year: 2022, region: "Lazio", limit: 2, offset: 2 }));
  assert.equal(body.pagination.total, 378);
  assert.equal(body.referenceYear, 2022);
  assert.equal(body.family, "FC80ISTRUZ");
  assert.equal(body.function, "ISTRUZIONE");
  assert.equal(body.modifiedAt, "2025-06-16");
  assert.equal(body.provenance.sha256.data, "a4f1f98f4dc070221798e8b2b2264a7254d27c1beac534355c96304755c861c0");
  assert.equal(descriptor.sources[0].sha256, body.provenance.sha256.data);
  assert.equal(queryOpenCivitas2022Istruzione({ code: "999999" }).pagination.total, 0);
  assert.equal(queryOpenCivitas2022Istruzione({ code: "010022" }).pagination.total, 0);
});
