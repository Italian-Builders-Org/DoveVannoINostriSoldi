...
`src/lib/studies.ts` espone la capsula dello studio e il commit dei materiali.
`src/content/papers/published/index.ts` la integra nel catalogo generale
`papers.listPublished()` usato dall’archivio `/studi`; `/paper` è un alias 308.
Le pagine sono Server Components
...
Test Python coprono classificazione degli stati, Kaplan-Meier con ties e censura
e pubblicazione idempotente senza sovrascrivere asset diversi.
