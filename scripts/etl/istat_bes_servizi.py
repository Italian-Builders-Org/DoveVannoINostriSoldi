#!/usr/bin/env python3
"""Build/check the separately locked BesT BES_12 services-quality domain, without network I/O."""
...
SPEC = ROOT / "scripts/etl/specs/istat-bes-servizi-2004-2024.source.json"
DATA = ROOT / "src/data/generated/istat-bes-servizi-2004-2024.data.json"
META = ROOT / "src/data/generated/istat-bes-servizi-2004-2024.meta.json"
DATASET = "istat-bes-servizi"


class SnapshotError(ValueError):
    """Source bytes or public services-quality-domain contract differ from the reviewed lock."""
...
def load_spec(path: Path = SPEC) -> dict:
    ...
    require(
        spec["domain"] == {"code": "BES_12", "label": "Qualità dei servizi", "edition": "2025"},
        "domain differs",
    )
...
def value_tenths(raw: str) -> int:
    try:
        value = Decimal(raw)
        require(value.is_finite() and value >= 0, "invalid services-quality-domain value")
        scaled = value * 10
        require(scaled == scaled.to_integral_value(), "value has more than one decimal")
        return int(scaled)
    except (InvalidOperation, ValueError) as error:
        raise SnapshotError(f"invalid services-quality-domain value: {raw!r}") from error
...
def build_data(payload: bytes, spec: dict) -> dict:
    ...
    for row in reader:
        ...
        code = row["DATA_TYPE"]
        require(code in indicators, "indicator outside services-quality domain")
...
def validate_data(data: dict, spec: dict) -> None:
    ...
    require(len(territories) == 139 and len(indicators) == 8, "duplicate/missing dictionary entry")
