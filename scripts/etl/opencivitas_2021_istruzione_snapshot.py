#!/usr/bin/env python3
"""Pinned FC70ISTRUZ 2021 release — Istruzione, not total services and not the 2022 release."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2021-istruzione.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2021-istruzione.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "a453f3e66123f1561f3ea559403f43fe287483350029aec2c073a0b0daea24ee"
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        "schemaVersion": 1,
        "transformVersion": 1,
        "scope": "ordinary-statute-municipalities-education-fc70-2021",
        "referenceYear": 2021,
        ...
        "source": {
            ...
            "dataset": "Comuni · Istruzione · Indicatori e determinanti 2021 (FC70ISTRUZ)",
        ...
        "methodology": {
            ...
            "coverageWarning": 'RSS e Province autonome fuori perimetro; esclusi gli aggregati di Italia, regioni, aree e fasce. Esclusi 15 Comuni con SPESA_STORICA o SPESA_STORICA_PROAB vuoti nella fonte (fabbisogno presente): nessuna imputazione a zero. Escluso Fascia (ISTAT 010022) con spesa storica e fabbisogno a zero nella fonte: un fabbisogno non positivo non viene pubblicato.',
            "rankingWarning": "I livelli della fonte non costituiscono un ranking di efficienza.",
            "functionSeparationWarning": "FC70ISTRUZ 2021 è distinto da FC70TOT 2021 (servizi totali) e dalla stessa funzione nel 2022: nessuna somma o confronto silenzioso fra funzioni o annualità.",
