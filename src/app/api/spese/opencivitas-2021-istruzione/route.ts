...
import { queryOpenCivitas2021Istruzione } from "@/lib/opencivitas-2021-istruzione-snapshot";
...
export function GET(request: NextRequest) {
  ...
  if (year !== null && year !== "2021") {
    return Response.json(
      { error: "Questo endpoint serve solo l'annualità 2021 FC70ISTRUZ. I servizi totali 2021 restano su /api/spese/opencivitas-2021." },
      { status: 400, headers: { "Cache-Control": "no-store" } },
    );
  }
  ...
  try {
    return Response.json(
      queryOpenCivitas2021Istruzione({
