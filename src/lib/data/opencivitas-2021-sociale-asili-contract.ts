...
export type OpenCivitas2021SocialeAsiliSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "afcce17b6257fbd8d2eb04060fd59f163cff5a326a4990b548c72ad6955c195c";
...
export function assertOpenCivitas2021SocialeAsiliSnapshot(value: unknown): OpenCivitas2021SocialeAsiliSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC70SOCNID: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC70SOCNID: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC70SOCNID: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC70SOCNID: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC70SOCNID: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2021SocialeAsiliSnapshot;
}
