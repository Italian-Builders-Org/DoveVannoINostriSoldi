...
import { queryOpenCivitas2017 } from "@/lib/opencivitas-2017-snapshot";
...
export function GET(request: NextRequest) {
  ...
  if (year !== null && year !== "2017") {
    return Response.json(
      { error: "Questo endpoint serve solo l'annualità 2017 FC40TOT. Le altre annualità restano nei rispettivi dataset." },
      { status: 400, headers: { "Cache-Control": "no-store" } },
    );
  }
  ...
  try {
    return Response.json(queryOpenCivitas2017({
