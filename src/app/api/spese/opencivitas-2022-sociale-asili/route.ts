...
import { queryOpenCivitas2022SocialeAsili } from "@/lib/opencivitas-2022-sociale-asili-snapshot";
...
export function GET(request: NextRequest) {
  ...
  if (year !== null && year !== "2022") {
    return Response.json(
      { error: "Questo endpoint serve solo l'annualità 2022 FC80SOCNID. I servizi totali restano su /api/spese/opencivitas." },
      { status: 400, headers: { "Cache-Control": "no-store" } },
    );
  }
  ...
  try {
    return Response.json(
      queryOpenCivitas2022SocialeAsili({
