import "server-only";
import snapshotJson from "@/data/generated/opencivitas-2022-polizia.json";
import {
  assertOpenCivitas2022PoliziaSnapshot,
  type OpenCivitas2022PoliziaSnapshot,
} from "@/lib/data/opencivitas-2022-polizia-contract";
import { resolveOpenCivitasRegionName } from "@/lib/region-query";

export const openCivitas2022PoliziaSnapshot: OpenCivitas2022PoliziaSnapshot =
  assertOpenCivitas2022PoliziaSnapshot(snapshotJson);

export function queryOpenCivitas2022Polizia(filters: {
  region?: string;
  code?: string;
  limit?: number;
  offset?: number;
  ...
  if (filters.region === undefined && filters.code === undefined) {
    throw new Error(
      "Specificare regione o codice ISTAT Comune: lo snapshot Polizia locale 2022 completo non viene servito in un'unica risposta.",
    );
  }
  ...
  const matches = openCivitas2022PoliziaSnapshot.municipalities.filter(
  ...
  return {
    datasetId: "opencivitas_polizia_2022",
    family: "FC80POLIZIA",
    function: "POLIZIA",
    referenceYear: openCivitas2022PoliziaSnapshot.referenceYear,
    publishedAt: openCivitas2022PoliziaSnapshot.publishedAt,
    modifiedAt: openCivitas2022PoliziaSnapshot.modifiedAt,
    pagination: {
    ...
    coverage: openCivitas2022PoliziaSnapshot.coverage,
    methodology: openCivitas2022PoliziaSnapshot.methodology,
    provenance: openCivitas2022PoliziaSnapshot.source,
    caveats: [
