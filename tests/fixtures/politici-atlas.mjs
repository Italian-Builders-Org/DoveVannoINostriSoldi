...
export function makeMap({ cameraCount = 398, senateCount = 205, vacancies = 2 } = {}) {
    ...
    for (let index = 0; index < familyLabels.length && assigned < total; index++) {
      ...
      for (let member = 0; member < size; member++) {
        ...
        people.push({ id, name: serial < named.length ? `${named[serial]}${chamberId === "senato" ? " (Senato)" : ""}` : `Persona ${chamberId} ${String(serial + 1).padStart(3, "0")}`,
          ...
          firstTerm: { chamber: serial % 2 === 1, parliament: serial % 4 === 1 }, groupChanges: serial % 3 === 2 ? 1 : 0 });
      }
  ...
  people.push({ id: "gov-1", name: "Elena Verdi", tier: 1, roleKind: "presidente-del-consiglio", roleLabel: "Presidente del Consiglio", chamberId: null, groupId: null, family: null, weight: 1, government: true, leader: true, groupLeader: false, photo: false, firstTerm: null, groupChanges: null });
  people.push({ id: "pres-1", name: "Andrea Bianchi", tier: 0, roleKind: "capo-stato", roleLabel: "Presidente della Repubblica", chamberId: null, groupId: null, family: null, weight: 1, government: false, leader: true, groupLeader: false, photo: false, firstTerm: null, groupChanges: null });
...
export function makeProfiles(map) {
  return Object.fromEntries(map.people.map((person) => [person.id, {
    ...
    groupHistory: person.groupChanges === null ? null : { chamber: person.chamberId, changes: person.groupChanges, entries: [
      ...(person.groupChanges ? [{ label: "Gruppo precedente di prova", laterLabels: [], startDate: "2022-10-18", endDate: "2024-03-01", joinedAtFormation: false }] : []),
      { label: "Gruppo di prova", laterLabels: [], startDate: person.groupChanges ? "2024-03-01" : "2022-10-18", endDate: null, joinedAtFormation: false },
    ], sourceLabel: "Fonte di prova — SPARQL", sourceUrl: "https://dati.camera.it/", observedDate: "2026-09-23", caveat: "Dati sintetici per i test." },
  }]));
}
