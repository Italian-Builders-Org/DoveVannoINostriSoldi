#!/usr/bin/env python3
"""Pinned FC80SOCNID 2022 release — Sociale e asili nido function, not total services."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2022-sociale-asili.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2022-sociale-asili.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "2646a7da3cb0f837e79e9b1d0a22cfc09f8a165a760a4658dc113d0b21deafe9"
...
def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """FC80SOCNID CSV includes Privacy (unlike FC80RIFIUTI)."""
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        "schemaVersion": 1,
        "transformVersion": 1,
        "scope": "ordinary-statute-municipalities-social-nursery-fc80-2022",
        "referenceYear": 2022,
        ...
        "source": {
            ...
            "dataset": "Comuni · Sociale e asili nido · Indicatori e determinanti 2022 (FC80SOCNID)",
        ...
        "methodology": {
            ...
            "coverageWarning": "RSS e Province autonome fuori perimetro; esclusi gli aggregati di Italia, regioni, aree e fasce. Esclusi 3 Comuni con SPESA_STORICA o SPESA_STORICA_PROAB vuoti nella fonte (FST presente): nessuna imputazione a zero.",
            "rankingWarning": "I livelli della fonte non costituiscono un ranking di efficienza.",
            "functionSeparationWarning": "FC80SOCNID 2022 è distinto da FC80TOT 2022, FC80RIFIUTI 2022, FC80TERRVIAB 2022 e dalle altre funzioni: nessuna somma o confronto silenzioso fra funzioni o annualità.",
        },
    }
