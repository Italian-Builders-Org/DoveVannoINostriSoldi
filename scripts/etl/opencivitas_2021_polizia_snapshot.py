#!/usr/bin/env python3
"""Pinned FC70POLIZIA 2021 release — Polizia locale, not total services and not the 2022 release."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2021-polizia.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2021-polizia.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "b92132fbc9b811804b9d160efbd7b0cfbd8db5efe769225ec78536efe078d579"

FUNCTION = "POLIZIA"
FAMILY = "FC70POLIZIA"

# Descrizioni bloccate dai metadati di QUESTO rilascio. Non riusare le stringhe
# di un'altra funzione o annualità: la fonte cambia maiuscole e unità.
SELECTED_INDICATORS = {
...
def verify_definitions(definitions: list[dict]) -> None:
    ...
    if len(codes) != SPEC["indicatorDefinitionCount"] or len(set(codes)) != len(codes):
        raise StructuralError(f"{FAMILY}: metadati indicatori inattesi o duplicati")
...
def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """CSV UTF-8 con colonna Privacy. La codifica è dichiarata, non tentata."""
    ...
    if row_count != SPEC["csvRowCount"]:
        raise StructuralError(f"{FAMILY}: numero di righe CSV diverso dal lock")
    return selected
...
def normalize_rows(entities: dict, raw: dict) -> list[dict]:
    ...
    excluded_non_positive = set(SPEC["excludedNonPositiveStandard"])
    seen_incomplete: set[str] = set()
    seen_non_positive: set[str] = set()
    result = []
    for username in sorted(set(raw) & set(entities)):
        ...
        if incomplete_historical(rows):
            ...
            continue
        if non_positive_standard(rows):
            seen_non_positive.add(username)
            continue
    ...
    if seen_non_positive != excluded_non_positive:
        raise StructuralError(
            f"{FAMILY}: Comuni con fabbisogno non positivo diversi dal lock "
            f"(attesi {sorted(excluded_non_positive)}, trovati {sorted(seen_non_positive)})"
        )
...
def verify_national_totals(published: list[dict]) -> None:
    ...
    L'uguaglianza vale sull'insieme dei Comuni con spesa storica, più il fabbisogno
    dei Comuni esclusi perché privi di quella spesa. Uno scarto oltre la tolleranza
    di arrotondamento blocca il bundle invece di pubblicare un risparmio apparente.
    """
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        "schemaVersion": 1,
        "transformVersion": 1,
        "scope": "ordinary-statute-municipalities-local-police-fc70-2021",
        "referenceYear": 2021,
        ...
        "source": {
            ...
            "dataset": "Comuni · Polizia locale · Indicatori e determinanti 2021 (FC70POLIZIA)",
        ...
        "methodology": {
            "differenceMeaning": "Differenza tra spesa storica e spesa standard sulla funzione Polizia locale. Non è una misura di spreco.",
            "serviceMeaning": "Il confronto sui servizi usa la media dei Comuni della stessa fascia di popolazione, perimetro Polizia locale.",
            "perCapitaMeaning": "Euro per abitante pubblicati dalla fonte; nessuna popolazione ricostruita o imputata.",
            "coverageWarning": 'RSS e Province autonome fuori perimetro; esclusi gli aggregati di Italia, regioni, aree e fasce. Esclusi 9 Comuni con SPESA_STORICA o SPESA_STORICA_PROAB vuoti nella fonte (fabbisogno presente): nessuna imputazione a zero.',
            "rankingWarning": "I livelli della fonte non costituiscono un ranking di efficienza.",
            "functionSeparationWarning": "FC70POLIZIA 2021 è distinto da FC70TOT 2021 (servizi totali) e dalla stessa funzione nel 2022: nessuna somma o confronto silenzioso fra funzioni o annualità.",
...
def main() -> None:
    ...
    if args.check:
        ...
        print(f"{FAMILY} 2021: snapshot verificato offline")
        return
    ...
    if args.output.exists() and SEMANTIC_SHA256 != "PENDING":
        ...
        if semantic_digest(current) == digest:
            print(f"{FAMILY} 2021: nessuna variazione")
            return
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_bytes(snapshot_text(result).encode("utf-8"))
    print(f"{FAMILY} 2021: {len(result['municipalityRows'])} Comuni")
