...
import { z } from "zod";
import sourceLock from "../../../scripts/etl/specs/istat-bes-lavoro-2008-2024.source.json";
...
const locked = <T>(expected: T) => z.custom<T>((value) => canonical(value) === canonical(expected), "Campo diverso dal lock BesT Lavoro");
const observationSchema = z.object({
  indicator: z.string(),
  territory: z.string(),
  sex: z.enum(["F", "M", "T"]),
  year: z.number().int().min(2008).max(2024),
...
const dataSchema = z.object({
  ...
  datasetId: z.literal("istat-bes-lavoro"),
...
export type IstatBesLavoroData = z.infer<typeof dataSchema>;

export function validateIstatBesLavoroBundle(data: unknown, metadata: unknown) {
  ...
  if (digest(normalizedLock) !== integrity.sourceLockSha256) {
    throw new Error("Snapshot BesT Lavoro: hash del source lock diverso.");
  }
  ...
  if (digest(parsed) !== integrity.dataArtifact.sha256 || Buffer.byteLength(serialized) !== integrity.dataArtifact.bytes) {
    throw new Error("Snapshot BesT Lavoro: hash o dimensione dell'artefatto diversi dal lock.");
  }
  const expectedMetadata = {
    schemaVersion: 1,
    datasetId: "istat-bes-lavoro",
  ...
  if (canonical(metadata) !== canonical(expectedMetadata)) {
    throw new Error("Snapshot BesT Lavoro: metadati diversi da fonte, semantica o integrità dichiarate.");
  }
  ...
  for (const row of parsed.observations) {
    ...
    if (!indicator || !territories.has(row.territory) || seen.has(key)
      ...
      throw new Error("Snapshot BesT Lavoro: chiave duplicata o dimensione fuori contratto.");
    }
    seen.add(key);
    if ((row.valueTenths === null) !== (row.status === "g")) {
      throw new Error("Snapshot BesT Lavoro: null e flag di dato ignoto incoerenti.");
    }
  ...
  if (canonical(nulls) !== canonical(sourceLock.nullCells)) {
    throw new Error("Snapshot BesT Lavoro: identità della cella ignota diversa.");
  }
  for (const indicator of parsed.indicators) {
    for (const [slice, count] of Object.entries(indicator.coverage)) {
      if (coverage.get(`${indicator.code}/${slice}`) !== count) {
        throw new Error("Snapshot BesT Lavoro: copertura per indicatore, sesso e anno diversa.");
      }
    }
  }
