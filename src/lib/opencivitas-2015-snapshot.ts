...
import {
  assertOpenCivitas2015Snapshot,
  type OpenCivitas2015Snapshot,
} from "@/lib/data/opencivitas-2015-contract";
import { resolveOpenCivitasRegionName } from "@/lib/region-query";

export const openCivitas2015Snapshot: OpenCivitas2015Snapshot =
  assertOpenCivitas2015Snapshot(readJsonSnapshot(
    join(process.cwd(), "src/data/generated/opencivitas-2015.json"),
    2 * 1024 * 1024,
  ));

export function queryOpenCivitas2015(filters: {
  region?: string;
  code?: string;
  limit?: number;
  offset?: number;
  ...
  if (filters.region === undefined && filters.code === undefined) {
    throw new Error("Specificare regione o codice ISTAT Comune: lo snapshot 2015 completo non viene servito in un'unica risposta.");
  }
  ...
  const matches = openCivitas2015Snapshot.municipalities.filter(
  ...
  return {
    datasetId: "opencivitas_fabbisogni_2015",
    family: "FC20TOT",
    referenceYear: openCivitas2015Snapshot.referenceYear,
    publishedAt: openCivitas2015Snapshot.publishedAt,
    modifiedAt: openCivitas2015Snapshot.modifiedAt,
    pagination: { total: matches.length, offset, limit, returned: data.length },
    data,
    coverage: openCivitas2015Snapshot.coverage,
    methodology: openCivitas2015Snapshot.methodology,
    provenance: openCivitas2015Snapshot.source,
    caveats: [
      "Snapshot distinto dalle annualità OpenCivitas 2017, 2018, 2019, 2021 e 2022.",
      "Non sommare né confrontare in silenzio le annualità.",
      "Nel 2015 il fabbisogno standard è riproporzionato sul totale nazionale della spesa storica: la differenza aggregata è nulla per costruzione e non va letta come risultato.",
