#!/usr/bin/env python3
"""Pinned FC70AMMIN 2021 release — Amministrazione function, not total services."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2021-amministrazione.source.json"
...
# con la d minuscola, mentre in FC70SOCNID sono "Da 0 a 10". Non riusare le stringhe
...
def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """FC70AMMIN CSV: cinque colonne con Privacy, codifica dichiarata nel lock."""
    raw_csv = read_outer_file(payload, ".csv")
    # Il rilascio 2021 pubblica il CSV in cp1252, il 2022 in UTF-8: la codifica e'
    # dichiarata nel lock e non dedotta. Nessun ripiego automatico fra le due.
    reader = csv.DictReader(io.TextIOWrapper(io.BytesIO(raw_csv), encoding=SPEC["csvEncoding"], newline=""), delimiter=";")
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        ...
        "source": {
            ...
            "dataset": "Comuni · Amministrazione · Indicatori e determinanti 2021 (FC70AMMIN)",
        ...
        "methodology": {
            ...
            "coverageWarning": "RSS e Province autonome fuori perimetro; esclusi gli aggregati di Italia, regioni, aree e fasce. Esclusi 15 Comuni con SPESA_STORICA o SPESA_STORICA_PROAB vuoti nella fonte (FST presente): nessuna imputazione a zero.",
            "rankingWarning": "I livelli della fonte non costituiscono un ranking di efficienza.",
            "functionSeparationWarning": "FC70AMMIN 2021 è distinto da FC70TOT 2021 (servizi totali), da FC80AMMIN 2022 (stessa funzione, altra annualità) e dalle altre funzioni: nessuna somma o confronto silenzioso fra funzioni o annualità.",
