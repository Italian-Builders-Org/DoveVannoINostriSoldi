...
import { queryOpenCivitas2016 } from "@/lib/opencivitas-2016-snapshot";
...
export function GET(request: NextRequest) {
  ...
  if (year !== null && year !== "2016") {
    return Response.json(
      { error: "Questo endpoint serve solo l'annualità 2016 FC30TOT. Le altre annualità restano nei rispettivi dataset." },
      { status: 400, headers: { "Cache-Control": "no-store" } },
    );
  }
  ...
  try {
    return Response.json(queryOpenCivitas2016({
