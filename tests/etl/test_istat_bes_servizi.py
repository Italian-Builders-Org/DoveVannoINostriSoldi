"""BES_12-specific SDMX parsing, coverage and provenance checks, entirely offline."""
import copy
import csv
import io
import json
...
import istat_bes_servizi as bes


class BesServiziTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        for observation in cls.data["observations"]:
            ...
            if value is None:
                ...
                obs_value = f"{value // 10}.{value % 10}".rstrip("0").rstrip(".")
                if obs_value == "":
                    obs_value = "0"
    ...
    def test_domain_units_and_edition_are_locked(self):
        for column, bad in [("UNIT_MEAS", "EURO"), ("DOMAIN", "BES_09"), ("EDITION", "2024"),
                            ("DATAFLOW", "IT1:DF_BES_TERRIT_9(1.0)")]:
    ...
    def test_null_flags_absent_row_and_observed_zero_stay_distinct(self):
        flagged = [row for row in self.data["observations"] if row["status"]]
        self.assertEqual(len(flagged), 76)
        self.assertEqual({row["status"] for row in flagged}, {"n", "g"})
        self.assertEqual(bes.value_tenths("0"), 0)
        self.assertEqual(bes.value_tenths("1.5"), 15)
