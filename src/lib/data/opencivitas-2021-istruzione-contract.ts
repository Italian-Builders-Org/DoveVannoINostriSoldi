...
export type OpenCivitas2021IstruzioneSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "a453f3e66123f1561f3ea559403f43fe287483350029aec2c073a0b0daea24ee";
...
export function assertOpenCivitas2021IstruzioneSnapshot(value: unknown): OpenCivitas2021IstruzioneSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC70ISTRUZ: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC70ISTRUZ: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC70ISTRUZ: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC70ISTRUZ: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC70ISTRUZ: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2021IstruzioneSnapshot;
}
