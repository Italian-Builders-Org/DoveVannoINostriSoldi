...
function Bars({ title, unit, rows, note, id }: { title: string; unit: string; rows: {label: string; value: number}[]; note: string; id: string }) {
  const max = unit==='%' || unit.startsWith('euro ogni 100') ? 100 : Math.max(...rows.map(r=>r.value), 1);
  ...
  return <main className={`shell ${styles.report}`}>
    ...
    <article>
      <header className={styles.hero}>
        ...
        <div className={styles.actions}><a className={styles.primaryAction} href="#soldi-in-entrata">Segui i soldi</a><a href={report.pdfPath} download>Scarica il rapporto PDF</a><a href="#riscontri">Vai ai riscontri</a></div>
      </header>
      ...
      <section id="riscontri" className={styles.section} aria-labelledby="riscontri-title"><div className={styles.sectionHeading}><p className={styles.eyebrow}>04 · Dalle anomalie alle evidenze</p><h2 id="riscontri-title">Che cosa non funziona, e perché</h2><p className={styles.prose}>Ora seguiamo i casi: prima i costi evitabili, poi i risultati mancati, i crediti e gli errori nei documenti. Ogni importo conserva la propria data e il proprio significato.</p></div>
