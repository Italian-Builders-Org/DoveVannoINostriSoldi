...
export async function queryPublicDataset(
  ...
  switch (query.dataset) {
    ...
    case "opencivitas_amministrazione_2021": {
      const { queryOpenCivitas2021Amministrazione } = await import("@/lib/opencivitas-2021-amministrazione-snapshot");
      if (query.year !== undefined && query.year !== 2021) {
        throw new Error("OpenCivitas FC70AMMIN è disponibile per il 2021. I servizi totali restano su opencivitas_fabbisogni.");
      }
      return jsonSafe(queryOpenCivitas2021Amministrazione({
        region: query.region,
        code: query.code,
        limit: query.limit,
        offset: query.offset,
    ...
    case "opencivitas_fabbisogni_2017": {
      ...
      if (query.year !== undefined && query.year !== 2017) {
        throw new Error("OpenCivitas FC40TOT è disponibile per il 2017. Le altre annualità restano nei rispettivi dataset.");
      }
      return jsonSafe(queryOpenCivitas2017({
        region: query.region,
        code: query.code,
        limit: query.limit,
        offset: query.offset,
    ...
    case "istat_bes_lavoro": {
      options.signal?.throwIfAborted();
      const { queryIstatBesLavoro } = await import("@/lib/istat-bes-lavoro-snapshot");
      return jsonSafe({
        dataset: query.dataset,
        ...queryIstatBesLavoro({ territory: query.territory, year: query.year, indicator: query.measure,
          sex: query.sex, limit: query.limit, offset: query.offset }, options),
      });
    }
    case "istat_bes_relazioni": {
      options.signal?.throwIfAborted();
      const { queryIstatBesRelazioni } = await import("@/lib/istat-bes-relazioni-snapshot");
      return jsonSafe({
        dataset: query.dataset,
