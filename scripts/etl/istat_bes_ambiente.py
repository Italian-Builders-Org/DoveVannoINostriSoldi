#!/usr/bin/env python3
"""Build/check the separately locked BesT BES_10 environment domain, without network I/O."""
...
SPEC = ROOT / "scripts/etl/specs/istat-bes-ambiente-2004-2023.source.json"
DATA = ROOT / "src/data/generated/istat-bes-ambiente-2004-2023.data.json"
META = ROOT / "src/data/generated/istat-bes-ambiente-2004-2023.meta.json"
DATASET = "istat-bes-ambiente"


class SnapshotError(ValueError):
    """Source bytes or public environment-domain contract differ from the reviewed lock."""
...
def load_spec(path: Path = SPEC) -> dict:
    ...
    require(
        spec["domain"] == {"code": "BES_10", "label": "Ambiente", "edition": "2025"},
        "domain differs",
    )
...
def value_hundredths(raw: str) -> int:
    try:
        value = Decimal(raw)
        require(value.is_finite() and value >= 0, "invalid environment-domain value")
        scaled = value * 100
        require(scaled == scaled.to_integral_value(), "value has more than two decimals")
        return int(scaled)
    except (InvalidOperation, ValueError) as error:
        raise SnapshotError(f"invalid environment-domain value: {raw!r}") from error
...
def build_data(payload: bytes, spec: dict) -> dict:
    ...
    for row in reader:
        ...
        code = row["DATA_TYPE"]
        require(code in indicators, "indicator outside environment domain")
        ...
        if code == "10AMB018P":
            require(row["NOTE_DATA_TYPE_DESCR"] == "" and row["NOTE_DATA_TYPE_SOURCE"] == "",
                    "empty note references differ for 10AMB018P")
        else:
        ...
        require((flag == "g" and raw == "") or (flag == "" and raw != ""), "flag/value mismatch")
        ...
        observations.append({
            ...
            "valueHundredths": None if flag else value_hundredths(raw),
            "status": flag or None,
        })
    observations.sort(key=lambda item: (item["indicator"], item["territory"], item["sex"], item["year"]))
    ...
    data.update(
        scale={"factor": 100, "note": "Valori esatti in centesimi dell'unità dichiarata per ogni indicatore."},
...
def validate_data(data: dict, spec: dict) -> None:
    ...
    require(len(territories) == 139 and len(indicators) == 11, "duplicate/missing dictionary entry")
    for territory in territories.values():
        parent = territory["parent"]
        if parent is not None:
            require(parent in territories and territories[parent]["depth"] + 1 == territory["depth"], "territory hierarchy differs")
    ...
    for row in data["observations"]:
        code, territory, sex, year = (row[key] for key in ("indicator", "territory", "sex", "year"))
        key = (code, territory, sex, year)
        require(key not in seen, "duplicate observation")
        seen.add(key)
        require(code in indicators and territory in territories and sex in {"T"}, "unknown dimension")
        ...
        if value is None:
            require(row["status"] == "g", "null must preserve unknown-data flag")
    ...
    for code, indicator in indicators.items():
        ...
        actual = {key: count for (indicator_code, key), count in coverage.items() if indicator_code == code}
