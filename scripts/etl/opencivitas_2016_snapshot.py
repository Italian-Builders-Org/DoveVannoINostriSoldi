#!/usr/bin/env python3
"""Pinned FC30TOT 2016 release, independently verified from official metadata."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2016.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2016.json")
SEMANTIC_SHA256 = "45163242b0c3f31479def3d327c48bd64ab0c1e2f03b00a3b4342aad78f0d9ee"
...
def verify_bytes(payload: bytes, key: str) -> None:
    ...
    if len(payload) != expected["bytes"] or hashlib.sha256(payload).hexdigest() != expected["sha256"]:
        raise StructuralError(f"FC30TOT: byte/SHA-256 {key} diversi dal lock")
...
def verify_definitions(definitions: list[dict]) -> None:
    ...
    if len(codes) != SPEC["indicatorDefinitions"] or len(set(codes)) != len(codes):
        raise StructuralError("FC30TOT: metadati indicatori duplicati o copertura inattesa")
    ...
    for code, description in SELECTED_INDICATORS.items():
        ...
        if (row.get("VAR_IND_DES") != description
            ...
            raise StructuralError(f"FC30TOT: definizione/unità/perimetro inattesi per {code}")


def normalize_rows(entities: dict, raw: dict) -> list[dict]:
    unknown = set(raw) - set(entities)
    if unknown != set(SPEC["excludedAggregates"]):
        raise StructuralError("FC30TOT: join enti sconosciuti o aggregati inattesi")
    result = []
    for username in sorted(set(raw) & set(entities)):
        ...
        if entity["region"] not in SPEC["regionCounts"]:
            raise StructuralError(f"FC30TOT: Comune fuori RSO {username}")
        rows = raw[username]
        if set(rows) != set(SELECTED_INDICATORS):
            raise StructuralError(f"FC30TOT: indicatori comunali mancanti {username}")
        if any(row["anomaly"] or row["privacy"] for row in rows.values()):
            raise StructuralError(f"FC30TOT: anomalia/privacy inattesa {username}")
    ...
    if len(result) != SPEC["municipalities"] or Counter(row["region"] for row in result) != SPEC["regionCounts"]:
        raise StructuralError("FC30TOT: copertura RSO non riconciliata")
    codes = [row["istatCode"] for row in result]
    if len(set(codes)) != len(codes):
        raise StructuralError("FC30TOT: codice ISTAT duplicato")
...
def verify_national_totals(municipalities: list[dict]) -> None:
    """Il rilascio 2016 riproporziona il fabbisogno sul totale della spesa storica.
    ...
    if historical != expected["historicalSpendingCents"] or standard != expected["standardSpendingCents"]:
        raise StructuralError("FC30TOT: totali nazionali diversi dal lock")
    if abs(historical - standard) > expected["roundingToleranceCents"]:
        raise StructuralError("FC30TOT: il fabbisogno non riproporziona più sul totale della spesa storica")
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    if headers != SPEC["csvHeaders"]:
        raise StructuralError("FC30TOT: schema CSV inatteso")
    ...
    return {
        ...
        "referenceYear": 2016, "publishedAt": SPEC["publishedAt"],
        ...
        "source": {
            ...
            "dataset": "Comuni · Servizi totali · Indicatori e determinanti 2016 (FC30TOT)",
            ...
            **{key + "Url": item["url"] for key, item in SPEC["files"].items()},
            ...
            "family": "FC30TOT", "releaseVersion": SPEC["releaseVersion"],
        ...
        "methodology": {
            ...
            "sourceFormatNote": "Il rilascio 2016 pubblica il CSV in cp1252 con il punto come separatore decimale; dal 2017 la fonte usa UTF-8 e la virgola. Codifica e separatore sono dichiarati nel lock, mai dedotti.",
        },
    }
...
def validate_snapshot(snapshot: dict) -> None:
    if not isinstance(snapshot, dict) or not isinstance(snapshot.get("source"), dict):
        raise StructuralError("FC30TOT: oggetto snapshot e fonte attesi")
    ...
    if (not isinstance(observed, str) or observed != snapshot["source"].get("observedAt")
        ...
        raise StructuralError("FC30TOT: timestamp di acquisizione non coerenti")
    try:
        ...
        raise StructuralError("FC30TOT: timestamp ISO con fuso atteso") from error
    if semantic_digest(snapshot) != SEMANTIC_SHA256:
        raise StructuralError("FC30TOT: SHA-256 semantico diverso dal rilascio verificato")
...
def main() -> None:
    ...
    if args.check:
        ...
        print("FC30TOT 2016: snapshot verificato offline")
        return
    ...
    if args.output.exists():
        ...
        if semantic_digest(current) == semantic_digest(result):
            print("FC30TOT 2016: nessuna variazione")
            return
    ...
    print(f"FC30TOT 2016: {len(result['municipalityRows'])} Comuni")
