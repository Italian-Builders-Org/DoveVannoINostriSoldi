#!/usr/bin/env python3
"""Build/check the separately locked BesT BES_06 politics-and-institutions domain, without network I/O."""
...
SPEC = ROOT / "scripts/etl/specs/istat-bes-politica-2004-2024.source.json"
DATA = ROOT / "src/data/generated/istat-bes-politica-2004-2024.data.json"
META = ROOT / "src/data/generated/istat-bes-politica-2004-2024.meta.json"
DATASET = "istat-bes-politica"


class SnapshotError(ValueError):
    """Source bytes or public politics-and-institutions contract differ from the reviewed lock."""
...
def load_spec(path: Path = SPEC) -> dict:
    ...
    require(
        spec["domain"] == {"code": "BES_06", "label": "Politica e istituzioni", "edition": "2025"},
        "domain differs",
    )
    ...
    require(spec["flags"] == [], "unexpected observation flags")
    return spec
...
def value_tenths(raw: str) -> int:
    try:
        value = Decimal(raw)
        require(value.is_finite() and value >= 0, "invalid politics-and-institutions value")
        scaled = value * 10
        require(scaled == scaled.to_integral_value(), "value has more than one decimal")
        return int(scaled)
    except (InvalidOperation, ValueError) as error:
        raise SnapshotError(f"invalid politics-and-institutions value: {raw!r}") from error
...
def build_data(payload: bytes, spec: dict) -> dict:
    ...
    for row in reader:
        ...
        code = row["DATA_TYPE"]
        require(code in indicators, "indicator outside politics-and-institutions domain")
        for key, values in spec["attributes"].items():
            require(row[key] in values, f"unexpected {key}")
        raw, flag = row["OBS_VALUE"], row["OBS_STATUS"]
        require(flag == "", "unexpected observation status flag")
        ...
        observations.append({
            ...
            "valueTenths": None if raw == "" else value_tenths(raw),
            "status": None,
        })
...
def validate_data(data: dict, spec: dict) -> None:
    ...
    require(len(territories) == 139 and len(indicators) == 7, "duplicate/missing dictionary entry")
    ...
    for row in data["observations"]:
        ...
        require(row["status"] is None, "unexpected status in published observations")
        if value is None:
