...
import { queryOpenCivitas2021Amministrazione } from "@/lib/opencivitas-2021-amministrazione-snapshot";
...
export function GET(request: NextRequest) {
  ...
  if (year !== null && year !== "2021") {
    return Response.json(
      { error: "Questo endpoint serve solo l'annualità 2021 FC70AMMIN. I servizi totali restano su /api/spese/opencivitas." },
      { status: 400, headers: { "Cache-Control": "no-store" } },
    );
  }
  ...
  try {
    return Response.json(
      queryOpenCivitas2021Amministrazione({
