...
const { GET } = await import("../src/app/api/spese/opencivitas-2022-viabilita/route.ts");
const { queryOpenCivitas2022Viabilita } = await import("../src/lib/opencivitas-2022-viabilita-snapshot.ts");
const base = "http://localhost/api/spese/opencivitas-2022-viabilita";

test("FC80TERRVIAB HTTP rejects ambiguous, unbounded or cross-year requests without caching errors", () => {
...
test("FC80TERRVIAB API and MCP share pagination, provenance and region alias behavior", async () => {
  const { queryPublicDataset } = await import("../src/lib/mcp/datasets.ts");
  const { datasetCatalog } = await import("../src/lib/mcp/catalog.ts");
  const descriptor = datasetCatalog.find((item) => item.id === "opencivitas_viabilita_2022");
  ...
  assert.deepEqual(body, queryOpenCivitas2022Viabilita({ region: "Lazio", limit: 2, offset: 2 }));
  assert.deepEqual(body, await queryPublicDataset({ dataset: "opencivitas_viabilita_2022", year: 2022, region: "Lazio", limit: 2, offset: 2 }));
  ...
  assert.equal(queryOpenCivitas2022Viabilita({ code: "999999" }).pagination.total, 0);
  assert.equal(queryOpenCivitas2022Viabilita({ code: "078052" }).pagination.total, 0);
});
