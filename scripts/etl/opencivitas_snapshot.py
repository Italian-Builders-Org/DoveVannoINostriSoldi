#!/usr/bin/env python3
"""Build and validate the OpenCivitas 2022 municipal comparison snapshot."""
...
OUTPUT = Path("src/data/generated/opencivitas-2022.json")
...
def verify_indicators(payload: bytes) -> None:
    ...
    for row in row_dicts(xlsx_rows(xlsx, f"Indicatori_FC80TOT_{REFERENCE_YEAR}")):
    ...
    for code, description in SELECTED_INDICATORS.items():
        if definitions.get(code) != description:
            raise StructuralError(f"Metadati indicatore {code} cambiati o mancanti")
...
def normalize(data_payload: bytes, entities_payload: bytes, indicators_payload: bytes, generated_at: str) -> dict:
    ...
    if len(municipalities) != EXPECTED_MUNICIPALITIES:
        raise StructuralError(f"Copertura comunale inattesa: {len(municipalities)}")
    ...
    return {
        ...
        "source": {
            ...
            "dataset": "Comuni · Servizi totali · Indicatori e determinanti 2022",
            ...
            "platformCheckCadence": "Verifica del rilascio 2022 ogni 24 ore; nuove annualità dopo convalida",
        },
        "methodology": {
...
def validate_snapshot(snapshot: object) -> None:
    ...
    if snapshot.get("scope") != "ordinary-statute-municipalities-total-services":
    ...
    if not isinstance(rows, list) or len(rows) != EXPECTED_MUNICIPALITIES:
        raise StructuralError("Snapshot: elenco Comuni insufficiente")
    codes = []
    for index, row in enumerate(rows):
        ...
        item = dict(zip(MUNICIPALITY_COLUMNS, row, strict=True))
...
def file_or_download(path: Path | None, url: str, timeout: int) -> bytes:
...
def main() -> int:
    ...
    if any(provided) and not all(provided):
        parser.error("specificare insieme --data-zip, --entities-zip e --indicators-zip")
    if not any(provided):
        latest_year = discover_latest_total_services_year(args.timeout)
        if latest_year != REFERENCE_YEAR:
            raise StructuralError(
                f"OpenCivitas ha pubblicato l'annualità {latest_year}; "
                "convalidare il nuovo contratto prima dell'ingestione"
            )
    snapshot = normalize(
        file_or_download(args.data_zip, DATA_URL, args.timeout),
        file_or_download(args.entities_zip, ENTITIES_URL, args.timeout),
        file_or_download(args.indicators_zip, INDICATORS_URL, args.timeout),
        utc_now(),
    )
    ...
    rome = dict(zip(snapshot["municipalityColumns"], rome_row, strict=True))
