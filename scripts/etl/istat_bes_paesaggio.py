#!/usr/bin/env python3
"""Build/check the separately locked BesT BES_09 landscape domain, without network I/O."""
...
SPEC = ROOT / "scripts/etl/specs/istat-bes-paesaggio-2004-2023.source.json"
DATA = ROOT / "src/data/generated/istat-bes-paesaggio-2004-2023.data.json"
META = ROOT / "src/data/generated/istat-bes-paesaggio-2004-2023.meta.json"
DATASET = "istat-bes-paesaggio"


class SnapshotError(ValueError):
    """Source bytes or public landscape-domain contract differ from the reviewed lock."""
...
def load_spec(path: Path = SPEC) -> dict:
    ...
    require(
        spec["domain"] == {"code": "BES_09", "label": "Paesaggio e patrimonio culturale", "edition": "2025"},
        "domain differs",
    )
...
def value_hundredths(raw: str) -> int:
    try:
        value = Decimal(raw)
        require(value.is_finite() and value >= 0, "invalid landscape-domain value")
        scaled = value * 100
        require(scaled == scaled.to_integral_value(), "value has more than two decimals")
        return int(scaled)
    except (InvalidOperation, ValueError) as error:
        raise SnapshotError(f"invalid landscape-domain value: {raw!r}") from error
...
def build_data(payload: bytes, spec: dict) -> dict:
    ...
    for row in reader:
        ...
        code = row["DATA_TYPE"]
        require(code in indicators, "indicator outside landscape domain")
        ...
        require((flag in {"n", "g"} and raw == "") or (flag == "" and raw != ""), "flag/value mismatch")
...
def validate_data(data: dict, spec: dict) -> None:
    ...
    require(len(territories) == 139 and len(indicators) == 3, "duplicate/missing dictionary entry")
    ...
    for row in data["observations"]:
        ...
        if value is None:
            require(row["status"] in {"n", "g"}, "null must preserve unavailable-data flag")
