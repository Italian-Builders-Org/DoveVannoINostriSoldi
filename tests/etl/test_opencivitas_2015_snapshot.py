...
import opencivitas_2015_snapshot as snapshot
from opencivitas_common import StructuralError, decimal_value, load_raw_data

FC20_SAMPLE = json.loads(Path("tests/fixtures/opencivitas-2015-source-sample.json").read_text(encoding="utf-8"))
COMMITTED = json.loads(snapshot.OUTPUT.read_text(encoding="utf-8"))


class FC20ReleaseTests(unittest.TestCase):
    def test_definitions_keep_the_verified_25_definition_shape(self):
        snapshot.verify_definitions(FC20_SAMPLE["definitions"])
        for field, value in [
            ...
            definitions = copy.deepcopy(FC20_SAMPLE["definitions"])
    ...
    def test_join_rejects_rss_unknown_entities_missing_indicators_and_flags(self):
        ...
        with patch.dict(snapshot.SPEC, {
            ...
            entities = {username: copy.deepcopy(FC20_SAMPLE["entity"])}
            raw = {username: copy.deepcopy(FC20_SAMPLE["indicators"]), "ZZ999ITA001": {}}
    ...
    def test_declared_decimal_separator_is_not_guessed(self):
        """Il 2015 pubblica il punto: la virgola degli anni recenti deve fallire, e viceversa."""
    ...
    def test_declared_encoding_is_not_guessed(self):
        """Il CSV 2015 è cp1252: leggerlo come UTF-8 deve fallire invece di ripiegare."""
        payload = Path("tests/fixtures/opencivitas-2015-latin1-sample.zip").read_bytes()
    ...
    def test_byte_locks_and_entire_committed_artifact_fail_closed(self):
        ...
        for change in ("amount", "period", "license", "hash", "coverage", "encoding"):
            ...
            if change == "period": altered["referenceYear"] = 2016
    ...
    def test_standard_spending_is_reproportioned_on_historical_total(self):
        """Proprietà dichiarata del rilascio 2015: se cambia, il bundle si blocca."""
    ...
    def test_non_evaluable_municipalities_remain_source_coded(self):
        ...
        self.assertEqual(len(spending), 31)
        self.assertEqual(len(services), 49)
