...
test("the aggregate release proof closes the fixed public contract", async () => {
  ...
  assert.equal(release.datasets.sourceRows, 13_822_506);
  assert.equal(release.datasets.publicRows, 840_160);
...
test("all 87 datasets remain visible and only catalog dispositions decide row access", async () => {
  const overview = await view.getIntegratedDataOverview();
  assert.equal(overview.complete, true);
  assert.equal(overview.datasets.length, 87);
  assert.equal(overview.datasets.filter((dataset) => dataset.queryable).length, 65);
  assert.equal(overview.datasets.reduce((sum, dataset) => sum + dataset.sourceRows, 0), 13_822_506);
  assert.equal(overview.datasets.reduce((sum, dataset) => sum + dataset.publicRows, 0), 840_160);
...
test("every queryable artifact passes schema, hash, decompression and URL gates", async () => {
  ...
  assert.equal(checked.length, 65);
  assert.equal(
...
test("enumerating every queryable dataset does not retain all parsed row arrays", async () => {
  ...
  const program = `
    ...
    await (async () => {
      ...
      if (loaded.length !== 65) throw new Error("Unexpected queryable dataset count");
    })();
    await new Promise((resolve) => setImmediate(resolve));
    const after = collect();
    const diagnostics = loader.getIntegratedDatasetLoaderDiagnosticsForTests();
