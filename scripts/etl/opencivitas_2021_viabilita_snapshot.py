#!/usr/bin/env python3
"""Pinned FC70TERRVIAB 2021 release — Viabilità e territorio, distinct from FC70TOT and from FC80TERRVIAB 2022."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2021-viabilita.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2021-viabilita.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "df451762f85844113ca83c7a6faf194690728b7cbcd71849b436fde10ac18f62"
...
def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """FC70TERRVIAB CSV includes Privacy (unlike FC80RIFIUTI)."""
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        "schemaVersion": 1,
        "transformVersion": 1,
        "scope": "ordinary-statute-municipalities-roads-fc70-2021",
        "referenceYear": 2021,
        ...
        "source": {
            ...
            "dataset": "Comuni · Viabilità e territorio · Indicatori e determinanti 2021 (FC70TERRVIAB)",
        ...
        "methodology": {
            "differenceMeaning": "Differenza tra spesa storica e spesa standard sulla funzione Viabilità e territorio. Non è una misura di spreco.",
            "serviceMeaning": "Il confronto sui servizi usa la media dei Comuni della stessa fascia di popolazione, perimetro Viabilità e territorio.",
            "perCapitaMeaning": "Euro per abitante pubblicati dalla fonte; nessuna popolazione ricostruita o imputata.",
            "coverageWarning": "RSS e Province autonome fuori perimetro; esclusi gli aggregati di Italia, regioni, aree e fasce. Esclusi 14 Comuni con SPESA_STORICA o SPESA_STORICA_PROAB vuoti nella fonte (FST presente): nessuna imputazione a zero.",
            "rankingWarning": "I livelli della fonte non costituiscono un ranking di efficienza.",
            "functionSeparationWarning": "FC70TERRVIAB 2021 è distinto da FC70TOT 2021 (servizi totali) e da FC80TERRVIAB 2022: nessuna somma o confronto silenzioso fra funzioni o annualità.",
        },
    }
