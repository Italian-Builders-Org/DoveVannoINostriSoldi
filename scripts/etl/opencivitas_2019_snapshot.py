#!/usr/bin/env python3
"""Pinned FC60TOT 2019 release; no inferred 2020 data or cross-year totals."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2019.source.json"
SPEC = json.loads(SPEC_PATH.read_text())
OUTPUT = Path("src/data/generated/opencivitas-2019.json")
SEMANTIC_SHA256 = "c341ad7fc9529a4f6fbcb0c59fcde6c5e8d6f85063897bb721535904d1a2b7a9"
...
def verify_bytes(payload: bytes, key: str) -> None:
    ...
    if len(payload) != expected["bytes"] or hashlib.sha256(payload).hexdigest() != expected["sha256"]:
        raise StructuralError(f"FC60TOT: byte/SHA-256 {key} diversi dal lock")


def verify_indicators(payload: bytes) -> None:
    definitions = list(row_dicts(xlsx_rows(read_outer_file(payload, ".xlsx"), "Indicatori_FC60TOT_2019")))
...
def verify_definitions(definitions: list[dict]) -> None:
    ...
    if len(codes) != 27 or len(set(codes)) != len(codes):
        raise StructuralError("FC60TOT: metadati indicatori duplicati o copertura inattesa")
    ...
    for code, description in SELECTED_INDICATORS.items():
        ...
        if (row.get("VAR_IND_DES") != description or row.get("VAR_IND_FUNZIONE") != "TOTALE"
            ...
            raise StructuralError(f"FC60TOT: definizione/unità/perimetro inattesi per {code}")


def normalize_rows(entities: dict, raw: dict) -> list[dict]:
    unknown = set(raw) - set(entities)
    if unknown != set(SPEC["excludedAggregates"]):
        raise StructuralError("FC60TOT: join enti sconosciuti o aggregati inattesi")
    result = []
    for username in sorted(set(raw) & set(entities)):
        entity = dict(entities[username])
        # Both spellings occur in the official 2019 metadata; same RSO.
        if entity["region"] == "EMILIA ROMAGNA":
            entity["region"] = "EMILIA-ROMAGNA"
        if entity["region"] not in SPEC["regionCounts"]:
            raise StructuralError(f"FC60TOT: Comune fuori RSO {username}")
        rows = raw[username]
        if set(rows) != set(SELECTED_INDICATORS):
            raise StructuralError(f"FC60TOT: indicatori comunali mancanti {username}")
        ...
        if any(row["anomaly"] or row["privacy"] for row in rows.values()):
            raise StructuralError(f"FC60TOT: anomalia/privacy inattesa {username}")
    ...
    if len(result) != SPEC["municipalities"] or Counter(row["region"] for row in result) != SPEC["regionCounts"]:
        raise StructuralError("FC60TOT: copertura RSO non riconciliata")
    codes = [row["istatCode"] for row in result]
    if len(set(codes)) != len(codes):
        raise StructuralError("FC60TOT: codice ISTAT duplicato")
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    if headers != SPEC["csvHeaders"]:
        raise StructuralError("FC60TOT: schema CSV inatteso")
    ...
    return {
        ...
        "referenceYear": 2019, "publishedAt": SPEC["publishedAt"],
        ...
        "source": {
            ...
            "dataset": "Comuni · Servizi totali · Indicatori e determinanti 2019 (FC60TOT)",
        ...
        "methodology": {
            ...
            "yearSeparationWarning": "FC60TOT 2019, FC70TOT 2021 e FC80TOT 2022 restano separati: nessuna somma o confronto silenzioso. Il 2020 non è ricostruito.",
        },
    }
...
def validate_snapshot(snapshot: dict) -> None:
    if not isinstance(snapshot, dict) or not isinstance(snapshot.get("source"), dict):
        raise StructuralError("FC60TOT: oggetto snapshot e fonte attesi")
    ...
    if (not isinstance(observed, str) or observed != snapshot["source"].get("observedAt")
        ...
        raise StructuralError("FC60TOT: timestamp di acquisizione non coerenti")
    try:
        ...
        raise StructuralError("FC60TOT: timestamp ISO con fuso atteso") from error
    if semantic_digest(snapshot) != SEMANTIC_SHA256:
        raise StructuralError("FC60TOT: SHA-256 semantico diverso dal rilascio verificato")
...
def main() -> None:
    ...
    if args.check:
        ...
        print("FC60TOT 2019: snapshot verificato offline")
        return
    ...
    if args.output.exists():
        ...
        if semantic_digest(current) == semantic_digest(result):
            print("FC60TOT 2019: nessuna variazione")
            return
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(snapshot_text(result), encoding="utf-8")
    print(f"FC60TOT 2019: {len(result['municipalityRows'])} Comuni")
