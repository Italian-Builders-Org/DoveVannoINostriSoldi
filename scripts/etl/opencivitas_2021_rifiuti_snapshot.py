#!/usr/bin/env python3
"""Pinned FC70RIFIUTI 2021 release — Rifiuti function, not total services and not the 2022 release."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2021-rifiuti.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2021-rifiuti.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "799b774ba3dbde1d2757a4d5f79b235d920a62bc589f2bd58e5ea7835b9e6514"

# Function-specific codes (no _TOT suffix). Descriptions locked from indicator metadata.
SELECTED_INDICATORS = {
...
def verify_bytes(payload: bytes, key: str) -> None:
    ...
    if len(payload) != expected["bytes"] or hashlib.sha256(payload).hexdigest() != expected["sha256"]:
        raise StructuralError(f"FC70RIFIUTI: byte/SHA-256 {key} diversi dal lock")


def verify_definitions(definitions: list[dict]) -> None:
    codes = [row.get("VAR_IND_COD") for row in definitions]
    if len(set(codes)) != len(codes):
        raise StructuralError("FC70RIFIUTI: metadati indicatori duplicati")
    by_code = {row["VAR_IND_COD"]: row for row in definitions}
    money = {"FST_RIPROPORZIONATO_BI", "FST_RIPROPORZIONATO_BI_PROAB", "SPESA_STORICA", "SPESA_STORICA_PROAB"}
    for code, description in SELECTED_INDICATORS.items():
        row = by_code.get(code, {})
        expected_tip = "FS" if code in money else "LQP"
        if (
            row.get("VAR_IND_DES") != description
            or row.get("VAR_IND_FUNZIONE") != "RIFIUTI"
            or row.get("VAR_IND_LINGUA") != "IT"
            or row.get("VAR_IND_TIP") != expected_tip
        ):
            raise StructuralError(f"FC70RIFIUTI: definizione/unità/perimetro inattesi per {code}")


def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """FC70RIFIUTI CSV has Anomalia but no Privacy column."""
    ...
    required = {"USERNAME", "Indicatore/Determinante", "Valore", "Anomalia"}
    if reader.fieldnames is None or list(reader.fieldnames) != SPEC["csvHeaders"]:
        raise StructuralError("FC70RIFIUTI: schema CSV inatteso")
    if not required.issubset(reader.fieldnames):
        raise StructuralError(f"FC70RIFIUTI: colonne mancanti {sorted(required - set(reader.fieldnames))}")
    ...
    for row in reader:
        ...
        if code in bucket:
            raise StructuralError(f"FC70RIFIUTI: dati duplicati {row['USERNAME']} / {code}")
        bucket[code] = {
...
def normalize_rows(entities: dict, raw: dict) -> list[dict]:
    ...
    if unknown != set(SPEC["excludedAggregates"]):
        raise StructuralError("FC70RIFIUTI: join enti sconosciuti o aggregati inattesi")
    result = []
    for username in sorted(set(raw) & set(entities)):
        ...
        if entity["region"] not in SPEC["regionCounts"]:
            raise StructuralError(f"FC70RIFIUTI: Comune fuori RSO {username}")
        rows = raw[username]
        if set(rows) != set(SELECTED_INDICATORS):
            raise StructuralError(f"FC70RIFIUTI: indicatori comunali mancanti {username}")
        if any(row["anomaly"] or row["privacy"] for row in rows.values()):
            raise StructuralError(f"FC70RIFIUTI: anomalia/privacy inattesa {username}")
    ...
    if len(result) != SPEC["municipalities"] or Counter(row["region"] for row in result) != SPEC["regionCounts"]:
        raise StructuralError("FC70RIFIUTI: copertura RSO non riconciliata")
    codes = [row["istatCode"] for row in result]
    if len(set(codes)) != len(codes):
        raise StructuralError("FC70RIFIUTI: codice ISTAT duplicato")
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        "schemaVersion": 1,
        "transformVersion": 1,
        "scope": "ordinary-statute-municipalities-waste-fc70-2021",
        "referenceYear": 2021,
        ...
        "source": {
            ...
            "dataset": "Comuni · Rifiuti · Indicatori e determinanti 2021 (FC70RIFIUTI)",
        ...
        "methodology": {
            "differenceMeaning": "Differenza tra spesa storica e spesa standard sulla funzione Rifiuti. Non è una misura di spreco.",
            "serviceMeaning": "Il confronto sui servizi usa la media dei Comuni della stessa fascia di popolazione, perimetro Rifiuti.",
            ...
            "functionSeparationWarning": "FC70RIFIUTI 2021 è distinto da FC70TOT 2021 (servizi totali) e da FC80RIFIUTI 2022: nessuna somma o confronto silenzioso fra funzioni o annualità.",
        },
    }
...
def validate_snapshot(snapshot: dict) -> None:
    if not isinstance(snapshot, dict) or not isinstance(snapshot.get("source"), dict):
        raise StructuralError("FC70RIFIUTI: oggetto snapshot e fonte attesi")
    ...
    if (
        ...
        raise StructuralError("FC70RIFIUTI: timestamp di acquisizione non coerenti")
    try:
        date = datetime.fromisoformat(observed.replace("Z", "+00:00"))
        if date.tzinfo is None or date < datetime(2024, 5, 30, tzinfo=timezone.utc):
            raise ValueError("acquisizione precedente al rilascio")
    except ValueError as error:
        raise StructuralError("FC70RIFIUTI: timestamp ISO con fuso atteso") from error
    if SEMANTIC_SHA256 == "PENDING":
        raise StructuralError("FC70RIFIUTI: SEMANTIC_SHA256 non ancora fissato")
    if semantic_digest(snapshot) != SEMANTIC_SHA256:
        raise StructuralError("FC70RIFIUTI: SHA-256 semantico diverso dal rilascio verificato")
...
def main() -> None:
    ...
    if args.check:
        ...
        print("FC70RIFIUTI 2021: snapshot verificato offline")
        return
    ...
    if args.print_semantic_sha or SEMANTIC_SHA256 == "PENDING":
        print(f"FC70RIFIUTI semantic sha256={digest}")
    if SEMANTIC_SHA256 != "PENDING":
    ...
    if args.output.exists() and SEMANTIC_SHA256 != "PENDING":
        ...
        if semantic_digest(current) == digest:
            print("FC70RIFIUTI 2021: nessuna variazione")
            return
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(snapshot_text(result), encoding="utf-8")
    print(f"FC70RIFIUTI 2021: {len(result['municipalityRows'])} Comuni")
