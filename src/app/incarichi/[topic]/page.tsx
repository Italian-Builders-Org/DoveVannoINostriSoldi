...
export function generateStaticParams() {
  return getEditorialTopics("incarichi").map((topic) => ({ topic: topic.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ topic: string }> }): Promise<Metadata> {
  const { topic: slug } = await params;
  const topic = getEditorialTopic("incarichi", slug);
  if (!topic) return {};
  return { title: topic.title, description: topic.description };
}

export default async function AppointmentsTopicRoute({ params }: { params: Promise<{ topic: string }> }) {
  const { topic: slug } = await params;
  const topic = getEditorialTopic("incarichi", slug);
  if (!topic) notFound();
