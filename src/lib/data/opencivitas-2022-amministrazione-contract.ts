...
export type OpenCivitas2022AmministrazioneSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "7ea47114a1e5acdc77f765b7fa9b623a8e167fe2ff802b6be68e20bc5ad68011";
...
export function assertOpenCivitas2022AmministrazioneSnapshot(value: unknown): OpenCivitas2022AmministrazioneSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC80AMMIN: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC80AMMIN: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC80AMMIN: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC80AMMIN: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC80AMMIN: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2022AmministrazioneSnapshot;
}
