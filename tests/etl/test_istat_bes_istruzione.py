"""Education-specific SDMX parsing, coverage and provenance checks, entirely offline."""
import copy
import csv
import io
import json
...
import istat_bes_istruzione as bes


class BesIstruzioneTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_null_unknown_status_and_observed_zero_stay_distinct(self):
        ...
        self.assertEqual(flagged[0]["status"], "g")
        self.assertTrue(all(r["valueTenths"] is None for r in flagged))
        self.assertEqual(bes.value_tenths("0"), 0)
        for raw, flag in [("0", "g"), ("", ""), ("", "n"), ("", "c")]:
    ...
    def test_education_does_not_apply_total_between_sexes(self):
