...
import { queryIstatPovertaSogliaAssoluta } from "@/lib/istat-poverta-soglia-assoluta-snapshot";

const FILTERS = new Set(["territorio", "anno", "tipologia", "ampiezza", "limit", "offset"]);
  ...
  try {
    for (const key of params.keys()) {
      if (!FILTERS.has(key) || params.getAll(key).length !== 1) {
        throw new Error(
          "Usare soltanto territorio, anno, tipologia, ampiezza, limit e offset, una volta ciascuno.",
        );
      }
    }
    ...
    const result = queryIstatPovertaSogliaAssoluta({
      territory: params.get("territorio") ?? undefined,
      year: rawYear === null ? undefined : Number(rawYear),
      householdTypology: params.get("tipologia") ?? undefined,
      municipalitySize: params.get("ampiezza") ?? undefined,
