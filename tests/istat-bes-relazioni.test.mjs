...
test("large snapshot stays out of TypeScript inference and is traced into every server entry point", () => {
  const runtime = readFileSync(new URL("../src/lib/istat-bes-relazioni-snapshot.ts", import.meta.url), "utf8");
  assert.doesNotMatch(runtime, /import dataArtifact from/);
  assert.match(runtime, /openSync\(join\(process\.cwd\(\), ISTAT_BES_RELAZIONI_DATA_PATH\)/);
});
...
test("Next traces the runtime-loaded snapshot into every server entry point", async () => {
  ...
  for (const route of ["/api/territori/bes-relazioni", "/api/assistant/chat", "/mcp", "/api/mcp"]) {
    assert.ok(tracing[route].includes("src/data/generated/istat-bes-relazioni-2011-2024.data.json"), route);
  }
});
