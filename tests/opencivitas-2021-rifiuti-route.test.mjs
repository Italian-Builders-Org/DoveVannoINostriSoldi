...
const { GET } = await import("../src/app/api/spese/opencivitas-2021-rifiuti/route.ts");
const { queryOpenCivitas2021Rifiuti } = await import("../src/lib/opencivitas-2021-rifiuti-snapshot.ts");
const base = "http://localhost/api/spese/opencivitas-2021-rifiuti";

test("FC70RIFIUTI HTTP rejects ambiguous, unbounded or cross-year requests without caching errors", () => {
...
test("FC70RIFIUTI API and MCP share pagination, provenance and region alias behavior", async () => {
  const { queryPublicDataset } = await import("../src/lib/mcp/datasets.ts");
  const { datasetCatalog } = await import("../src/lib/mcp/catalog.ts");
  const descriptor = datasetCatalog.find((item) => item.id === "opencivitas_rifiuti_2021");
  ...
  assert.deepEqual(body, queryOpenCivitas2021Rifiuti({ region: "Lazio", limit: 2, offset: 2 }));
  assert.deepEqual(body, await queryPublicDataset({ dataset: "opencivitas_rifiuti_2021", year: 2021, region: "Lazio", limit: 2, offset: 2 }));
  assert.equal(body.pagination.total, 378);
  assert.equal(body.referenceYear, 2021);
  assert.equal(body.family, "FC70RIFIUTI");
  assert.equal(body.function, "RIFIUTI");
  assert.equal(body.modifiedAt, "2024-05-30");
  assert.equal(body.provenance.sha256.data, "ab8b417cd957394b25aa278d7f4a788d4a8a1931db8909f68f96a0479ab702f6");
  assert.equal(descriptor.sources[0].sha256, body.provenance.sha256.data);
  assert.equal(queryOpenCivitas2021Rifiuti({ code: "999999" }).pagination.total, 0);
});
