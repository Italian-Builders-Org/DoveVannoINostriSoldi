...
import { z } from "zod";
import sourceLock from "../../../scripts/etl/specs/istat-bes-sicurezza-2004-2023.source.json";
...
const locked = <T>(expected: T) => z.custom<T>((value) => canonical(value) === canonical(expected), "Campo diverso dal lock BesT Sicurezza");
...
const dataSchema = z.object({
  ...
  datasetId: z.literal("istat-bes-sicurezza"),
...
export type IstatBesSicurezzaData = z.infer<typeof dataSchema>;

export function validateIstatBesSicurezzaBundle(data: unknown, metadata: unknown) {
  ...
  if (digest(normalizedLock) !== integrity.sourceLockSha256) {
    throw new Error("Snapshot BesT Sicurezza: hash del source lock diverso.");
  }
  ...
  if (digest(parsed) !== integrity.dataArtifact.sha256 || Buffer.byteLength(serialized) !== integrity.dataArtifact.bytes) {
    throw new Error("Snapshot BesT Sicurezza: hash o dimensione dell'artefatto diversi dal lock.");
  }
  const expectedMetadata = {
    schemaVersion: 1,
    datasetId: "istat-bes-sicurezza",
  ...
  if (canonical(metadata) !== canonical(expectedMetadata)) {
    throw new Error("Snapshot BesT Sicurezza: metadati diversi da fonte, semantica o integrità dichiarate.");
  }
  ...
  for (const row of parsed.observations) {
    ...
    if (!indicator || !territories.has(row.territory) || seen.has(key)
      ...
      throw new Error("Snapshot BesT Sicurezza: chiave duplicata o dimensione fuori contratto.");
    }
    seen.add(key);
    if ((row.valueTenths === null) !== (row.status === "g")) {
      throw new Error("Snapshot BesT Sicurezza: null e flag di dato ignoto incoerenti.");
    }
  ...
  if (canonical(nulls) !== canonical(sourceLock.nullCells)) {
    throw new Error("Snapshot BesT Sicurezza: identità della cella non disponibile diversa.");
  }
  for (const indicator of parsed.indicators) {
    for (const [slice, count] of Object.entries(indicator.coverage)) {
      if (coverage.get(`${indicator.code}/${slice}`) !== count) {
        throw new Error("Snapshot BesT Sicurezza: copertura per indicatore, sesso e anno diversa.");
      }
    }
  }
