...
import { queryIstatPovertaRegioni } from "@/lib/istat-poverta-regioni-snapshot";

const FILTERS = new Set(["territorio", "misura", "anno", "limit", "offset"]);
  ...
  try {
    for (const key of params.keys()) {
      if (!FILTERS.has(key) || params.getAll(key).length !== 1) {
        throw new Error(
          "Usare soltanto territorio, misura, anno, limit e offset, una volta ciascuno.",
        );
      }
    }
    ...
    const result = queryIstatPovertaRegioni({
      territory: params.get("territorio") ?? undefined,
      measure: params.get("misura") ?? undefined,
