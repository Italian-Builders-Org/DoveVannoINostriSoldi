import "server-only";
import snapshotJson from "@/data/generated/opencivitas-2019.json";
import {
  assertOpenCivitas2019Snapshot,
  type OpenCivitas2019Snapshot,
} from "@/lib/data/opencivitas-2019-contract";
import { resolveOpenCivitasRegionName } from "@/lib/region-query";

export const openCivitas2019Snapshot: OpenCivitas2019Snapshot =
  assertOpenCivitas2019Snapshot(snapshotJson);

export function queryOpenCivitas2019(filters: {
  region?: string;
  code?: string;
  limit?: number;
  offset?: number;
  ...
  if (filters.region === undefined && filters.code === undefined) {
    throw new Error(
      "Specificare regione o codice ISTAT Comune: lo snapshot 2019 completo non viene servito in un'unica risposta.",
    );
  }
  ...
  const matches = openCivitas2019Snapshot.municipalities.filter(
  ...
  return {
    datasetId: "opencivitas_fabbisogni_2019",
    family: "FC60TOT",
    referenceYear: openCivitas2019Snapshot.referenceYear,
    publishedAt: openCivitas2019Snapshot.publishedAt,
    modifiedAt: openCivitas2019Snapshot.modifiedAt,
    pagination: {
    ...
    coverage: openCivitas2019Snapshot.coverage,
    methodology: openCivitas2019Snapshot.methodology,
    provenance: openCivitas2019Snapshot.source,
    caveats: [
      "Snapshot distinto da OpenCivitas 2021 FC70TOT e 2022 FC80TOT.",
