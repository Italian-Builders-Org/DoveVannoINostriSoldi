...
test("all 101 datasets remain visible and only catalog dispositions decide row access", async () => {
  const overview = await view.getIntegratedDataOverview();
  assert.equal(overview.complete, true);
  assert.equal(overview.datasets.length, 101);
  assert.equal(overview.datasets.filter((dataset) => dataset.queryable).length, 79);
...
test("every queryable artifact passes schema, hash, decompression and URL gates", async () => {
  ...
  assert.equal(checked.length, 79);
  assert.equal(
...
test("enumerating every queryable dataset does not retain all parsed row arrays", async () => {
  ...
  const program = `
    ...
    await (async () => {
      ...
      if (loaded.length !== 79) throw new Error("Unexpected queryable dataset count");
    })();
    await new Promise((resolve) => setImmediate(resolve));
    const after = collect();
    const diagnostics = loader.getIntegratedDatasetLoaderDiagnosticsForTests();
