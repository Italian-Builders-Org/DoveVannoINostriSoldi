...
test('BYOK planner receives all registered datasets within a compact metadata budget',async()=>{
  ...
  await executeByokChat(connection,messages,{signal:signal(),fetcher:async(url,init)=>{
    ...
    for(const dataset of datasetCatalog) {
      const compact=[dataset.id,dataset.title,dataset.filters,Object.fromEntries(Object.entries(dataset.exampleQuery).filter(([key])=>key!=="dataset"))];
      assert.ok(body.instructions.includes(JSON.stringify(compact)),`planner metadata missing for ${dataset.id}`);
    }
    assert.ok(body.instructions.length<17900,`catalog and query contract should remain compact: ${body.instructions.length} characters`);
