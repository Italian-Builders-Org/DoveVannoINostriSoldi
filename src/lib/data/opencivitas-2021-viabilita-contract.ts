...
export type OpenCivitas2021ViabilitaSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "df451762f85844113ca83c7a6faf194690728b7cbcd71849b436fde10ac18f62";
...
export function assertOpenCivitas2021ViabilitaSnapshot(value: unknown): OpenCivitas2021ViabilitaSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC70TERRVIAB: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC70TERRVIAB: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC70TERRVIAB: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC70TERRVIAB: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC70TERRVIAB: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2021ViabilitaSnapshot;
}
