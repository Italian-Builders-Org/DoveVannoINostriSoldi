...
const { GET } = await import("../src/app/api/spese/opencivitas-2021-polizia/route.ts");
const { queryOpenCivitas2021Polizia } = await import("../src/lib/opencivitas-2021-polizia-snapshot.ts");
const base = "http://localhost/api/spese/opencivitas-2021-polizia";

test("FC70POLIZIA HTTP rejects ambiguous, unbounded or cross-year requests without caching errors", () => {
...
test("FC70POLIZIA API and MCP share pagination, provenance and region alias behavior", async () => {
  const { queryPublicDataset } = await import("../src/lib/mcp/datasets.ts");
  const { datasetCatalog } = await import("../src/lib/mcp/catalog.ts");
  const descriptor = datasetCatalog.find((item) => item.id === "opencivitas_polizia_2021");
  ...
  assert.deepEqual(body, queryOpenCivitas2021Polizia({ region: "Lazio", limit: 2, offset: 2 }));
  ...
  assert.equal(body.provenance.sha256.data, "709732432e95a296625aaf9bf793dbcb2db502a864ee186769d81384a3c68dbd");
  assert.equal(descriptor.sources[0].sha256, body.provenance.sha256.data);
  assert.equal(queryOpenCivitas2021Polizia({ code: "999999" }).pagination.total, 0);
  assert.equal(queryOpenCivitas2021Polizia({ code: "066083" }).pagination.total, 0);
});
