...
test("composition component keeps partial state, keyboard tooltip and exact table in its contract", async () => {
  ...
  assert.doesNotMatch(css, /\.visual \{ display: none; \}/);
  ...
  assert.match(component, /Dati esatti e fonte/);
  assert.match(component, /Quote sul/);
  ...
  assert.match(css, /\.visual \{[\s\S]*?height: var\(--composition-height, clamp\(250px, 62cqw, 320px\)\);/);
...
test("home reading guidance is a compact accessible aside", async () => {
  ...
  assert.match(page, /<h2 id="reading-title" className="panel-title">\s*Perimetri diversi\s*<\/h2>/);
