#!/usr/bin/env python3
"""Build/check the separately locked BesT BES_02 education domain, without network I/O."""
...
SPEC = ROOT / "scripts/etl/specs/istat-bes-istruzione-2004-2024.source.json"
DATA = ROOT / "src/data/generated/istat-bes-istruzione-2004-2024.data.json"
META = ROOT / "src/data/generated/istat-bes-istruzione-2004-2024.meta.json"
DATASET = "istat-bes-istruzione"


class SnapshotError(ValueError):
    """Source bytes or public education contract differ from the reviewed lock."""
...
def load_spec(path: Path = SPEC) -> dict:
    ...
    require(spec["domain"] == {"code": "BES_02", "label": "Istruzione e formazione", "edition": "2025"}, "domain differs")
...
def value_tenths(raw: str) -> int:
    try:
        value = Decimal(raw)
        require(value.is_finite() and value >= 0, "invalid education value")
        scaled = value * 10
        require(scaled == scaled.to_integral_value(), "value has more than one decimal")
        return int(scaled)
    except (InvalidOperation, ValueError) as error:
        raise SnapshotError(f"invalid education value: {raw!r}") from error
...
def build_data(payload: bytes, spec: dict) -> dict:
    ...
    for row in reader:
        ...
        code = row["DATA_TYPE"]
        require(code in indicators, "indicator outside education domain")
...
def validate_data(data: dict, spec: dict) -> None:
    ...
    require(len(territories) == 139 and len(indicators) == 9, "duplicate/missing dictionary entry")
    ...
    return {"schemaVersion": 1, "datasetId": DATASET, "period": spec["period"],
            ...
            "semantics": spec["semantics"], "integrity": {
                "sourceLockSha256": lock_hash(spec), "dataArtifact": {
                    "path": DATA.relative_to(ROOT).as_posix(), "bytes": len(data_bytes),
