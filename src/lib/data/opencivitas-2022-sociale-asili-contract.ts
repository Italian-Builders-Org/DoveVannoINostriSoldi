...
export type OpenCivitas2022SocialeAsiliSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "2646a7da3cb0f837e79e9b1d0a22cfc09f8a165a760a4658dc113d0b21deafe9";
...
export function assertOpenCivitas2022SocialeAsiliSnapshot(value: unknown): OpenCivitas2022SocialeAsiliSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC80SOCNID: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC80SOCNID: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC80SOCNID: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC80SOCNID: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC80SOCNID: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2022SocialeAsiliSnapshot;
}
