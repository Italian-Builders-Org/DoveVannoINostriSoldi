...
**Stato:** il MCP espone uno snapshot aggregato verificato sui dodici file CIG 2025. Conserva copertura, hash, criteri di replica e limiti; non offre ancora ricerca live per singolo CIG, aggiudicatario o fornitore. Un [contratto separato](./research/ANAC_AWARDEES_COVERAGE.md) misura inoltre i full snapshot `aggiudicatari` e `aggiudicazioni`, la qualità del codice fiscale e il join esatto `CIG + id_aggiudicazione`. Il relativo manifest è soltanto aggregato e non abilita ancora UI, ranking o MCP. Il [consumer entity procurement](./research/ANAC_ENTITY_PROCUREMENT_COVERAGE.md) definisce invece la readiness del join interno AUSA/CF delle stazioni appaltanti, mantenendo separati importo aggiudicato e importo lotto; non pubblica identificatori ente, un entity index, un artefatto di pagina o una nuova API.
