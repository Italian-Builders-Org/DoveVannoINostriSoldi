import "server-only";
import snapshotJson from "@/data/generated/opencivitas-2022-amministrazione.json";
import {
  assertOpenCivitas2022AmministrazioneSnapshot,
  type OpenCivitas2022AmministrazioneSnapshot,
} from "@/lib/data/opencivitas-2022-amministrazione-contract";
import { resolveOpenCivitasRegionName } from "@/lib/region-query";

export const openCivitas2022AmministrazioneSnapshot: OpenCivitas2022AmministrazioneSnapshot =
  assertOpenCivitas2022AmministrazioneSnapshot(snapshotJson);

export function queryOpenCivitas2022Amministrazione(filters: {
  region?: string;
  code?: string;
  limit?: number;
  offset?: number;
  ...
  if (filters.region === undefined && filters.code === undefined) {
    throw new Error(
      "Specificare regione o codice ISTAT Comune: lo snapshot Amministrazione 2022 completo non viene servito in un'unica risposta.",
    );
  }
  ...
  const matches = openCivitas2022AmministrazioneSnapshot.municipalities.filter(
  ...
  return {
    datasetId: "opencivitas_amministrazione_2022",
    family: "FC80AMMIN",
    function: "AMMINISTRAZIONE",
    referenceYear: openCivitas2022AmministrazioneSnapshot.referenceYear,
    publishedAt: openCivitas2022AmministrazioneSnapshot.publishedAt,
    modifiedAt: openCivitas2022AmministrazioneSnapshot.modifiedAt,
    pagination: {
    ...
    coverage: openCivitas2022AmministrazioneSnapshot.coverage,
    methodology: openCivitas2022AmministrazioneSnapshot.methodology,
    provenance: openCivitas2022AmministrazioneSnapshot.source,
    caveats: [
