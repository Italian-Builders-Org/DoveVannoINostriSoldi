...
const fixture = { status: "published", slug: "studio-esempio", title: "Studio di esempio", abstract: "Una domanda verificabile.", authors: ["Autore di esempio"], publishedOn: "2026-09-06", version: 1, limitations: "Analisi descrittiva, non causale.", pdfUrl: "https://example.org/studio-v1.pdf", pdfSha256: "a".repeat(64), reproducibilityUrl: "https://example.org/studio/v1" };

test("paper: catalogo pubblico vuoto, bozze escluse e discovery completa", () => {
  assert.deepEqual(papers.listPublished(), []);
  assert.ok(PRIMARY_NAV.some((entry) => entry.href === "/paper"));
  assert.ok(SITE_MAP_GROUPS.some((group) => group.links.some((link) => link.href === "/paper")));
  assert.ok(PUBLIC_INDEXABLE_PATHS.includes("/paper"));
  assert.ok(LLMS_DISCOVERY_PATHS.includes("/paper"));
  assert.ok(searchSiteDocuments("paper").some((result) => result.href === "/paper"));
  ...
  assert.equal(searchSiteDocuments("Dai fondi ai posti").some((result) => result.href.startsWith("/paper#")), false);
});

test("paper: contratti editoriali e provenienza obbligatori", () => {
  assert.equal(validatePublishedPaper(fixture), fixture);
  for (const patch of [{ status: "draft" }, { publishedOn: "2026-02-30" }, { authors: [] }, { limitations: "" }, { pdfSha256: "bad" }, { pdfUrl: "file:///private/draft.pdf" }, { reproducibilityUrl: "http://localhost:3000" }, { version: 0 }, { slug: "../draft" }]) {
