#!/usr/bin/env python3
"""Pinned FC80RIFIUTI 2022 release — Rifiuti function, not total services."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2022-rifiuti.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2022-rifiuti.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "43175063f30e9f09a064801ab4a309e2464078fbcfe767171c371cff5c8f9dc2"
...
def verify_bytes(payload: bytes, key: str) -> None:
    ...
    if len(payload) != expected["bytes"] or hashlib.sha256(payload).hexdigest() != expected["sha256"]:
        raise StructuralError(f"FC80RIFIUTI: byte/SHA-256 {key} diversi dal lock")


def verify_definitions(definitions: list[dict]) -> None:
    codes = [row.get("VAR_IND_COD") for row in definitions]
    if len(set(codes)) != len(codes):
        raise StructuralError("FC80RIFIUTI: metadati indicatori duplicati")
    ...
    for code, description in SELECTED_INDICATORS.items():
        ...
        if (
            ...
            raise StructuralError(f"FC80RIFIUTI: definizione/unità/perimetro inattesi per {code}")


def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """FC80RIFIUTI CSV has Anomalia but no Privacy column."""
    ...
    if reader.fieldnames is None or list(reader.fieldnames) != SPEC["csvHeaders"]:
        raise StructuralError("FC80RIFIUTI: schema CSV inatteso")
    if not required.issubset(reader.fieldnames):
        raise StructuralError(f"FC80RIFIUTI: colonne mancanti {sorted(required - set(reader.fieldnames))}")
    ...
    for row in reader:
        ...
        if code in bucket:
            raise StructuralError(f"FC80RIFIUTI: dati duplicati {row['USERNAME']} / {code}")
        bucket[code] = {
...
def normalize_rows(entities: dict, raw: dict) -> list[dict]:
    ...
    if unknown != set(SPEC["excludedAggregates"]):
        raise StructuralError("FC80RIFIUTI: join enti sconosciuti o aggregati inattesi")
    result = []
    for username in sorted(set(raw) & set(entities)):
        ...
        if entity["region"] not in SPEC["regionCounts"]:
            raise StructuralError(f"FC80RIFIUTI: Comune fuori RSO {username}")
        rows = raw[username]
        if set(rows) != set(SELECTED_INDICATORS):
            raise StructuralError(f"FC80RIFIUTI: indicatori comunali mancanti {username}")
        if any(row["anomaly"] or row["privacy"] for row in rows.values()):
            raise StructuralError(f"FC80RIFIUTI: anomalia/privacy inattesa {username}")
    ...
    if len(result) != SPEC["municipalities"] or Counter(row["region"] for row in result) != SPEC["regionCounts"]:
        raise StructuralError("FC80RIFIUTI: copertura RSO non riconciliata")
    codes = [row["istatCode"] for row in result]
    if len(set(codes)) != len(codes):
        raise StructuralError("FC80RIFIUTI: codice ISTAT duplicato")
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        ...
        "source": {
            ...
            "dataset": "Comuni · Rifiuti · Indicatori e determinanti 2022 (FC80RIFIUTI)",
        ...
        "methodology": {
            ...
            "functionSeparationWarning": "FC80RIFIUTI 2022 è distinto da FC80TOT 2022 (servizi totali) e dalle altre funzioni: nessuna somma o confronto silenzioso fra funzioni o annualità.",
        },
    }
...
def validate_snapshot(snapshot: dict) -> None:
    if not isinstance(snapshot, dict) or not isinstance(snapshot.get("source"), dict):
        raise StructuralError("FC80RIFIUTI: oggetto snapshot e fonte attesi")
    ...
    if (
        ...
        raise StructuralError("FC80RIFIUTI: timestamp di acquisizione non coerenti")
    try:
        ...
        raise StructuralError("FC80RIFIUTI: timestamp ISO con fuso atteso") from error
    if SEMANTIC_SHA256 == "PENDING":
        raise StructuralError("FC80RIFIUTI: SEMANTIC_SHA256 non ancora fissato")
    if semantic_digest(snapshot) != SEMANTIC_SHA256:
        raise StructuralError("FC80RIFIUTI: SHA-256 semantico diverso dal rilascio verificato")
...
def main() -> None:
    ...
    if args.check:
        ...
        print("FC80RIFIUTI 2022: snapshot verificato offline")
        return
    ...
    if args.print_semantic_sha or SEMANTIC_SHA256 == "PENDING":
        print(f"FC80RIFIUTI semantic sha256={digest}")
    if SEMANTIC_SHA256 != "PENDING":
    ...
    if args.output.exists() and SEMANTIC_SHA256 != "PENDING":
        ...
        if semantic_digest(current) == digest:
            print("FC80RIFIUTI 2022: nessuna variazione")
            return
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(snapshot_text(result), encoding="utf-8")
    print(f"FC80RIFIUTI 2022: {len(result['municipalityRows'])} Comuni")
