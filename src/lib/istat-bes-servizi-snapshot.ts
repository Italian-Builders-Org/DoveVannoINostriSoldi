...
import metadataArtifact from "@/data/generated/istat-bes-servizi-2004-2024.meta.json";
import { validateIstatBesServiziBundle } from "@/lib/data/istat-bes-servizi-contract";

export const ISTAT_BES_SERVIZI_DATA_PATH = "src/data/generated/istat-bes-servizi-2004-2024.data.json";

const dataArtifact = readJsonSnapshot(join(process.cwd(), ISTAT_BES_SERVIZI_DATA_PATH), 4 * 1024 * 1024);
const validated = validateIstatBesServiziBundle(dataArtifact, metadataArtifact);
export const istatBesServiziData = validated.data;
export const istatBesServiziMetadata = validated.metadata;

export type IstatBesServiziQuery = Readonly<{
  territory?: string;
  year?: number;
  indicator?: string;
  sex?: string;
...
/** Both HTTP and MCP consume this validated services-quality selector. */
export function queryIstatBesServizi(query: IstatBesServiziQuery = {}, options: { signal?: AbortSignal } = {}) {
  ...
  const data = istatBesServiziData;
  ...
  if (indicator !== undefined && !data.indicators.some((entry) => entry.code === indicator)) {
    throw new Error("Indicatore non riconosciuto: usare un codice del dominio Qualità dei servizi, per esempio 12SER024.");
  }
  ...
  return {
    ...
    source: istatBesServiziMetadata.source,
    semantics: istatBesServiziMetadata.semantics,
