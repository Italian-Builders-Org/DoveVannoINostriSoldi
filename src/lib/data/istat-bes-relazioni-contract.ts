...
import { z } from "zod";
import sourceLock from "../../../scripts/etl/specs/istat-bes-relazioni-2011-2024.source.json";
...
const locked = <T>(expected: T) => z.custom<T>((value) => canonical(value) === canonical(expected), "Campo diverso dal lock BesT Relazioni sociali");
const observationSchema = z.object({
  indicator: z.string(),
  territory: z.string(),
  sex: z.literal("T"),
  year: z.number().int().min(2011).max(2024),
...
const dataSchema = z.object({
  ...
  datasetId: z.literal("istat-bes-relazioni"),
...
export type IstatBesRelazioniData = z.infer<typeof dataSchema>;

export function validateIstatBesRelazioniBundle(data: unknown, metadata: unknown) {
  ...
  if (digest(normalizedLock) !== integrity.sourceLockSha256) {
    throw new Error("Snapshot BesT Relazioni sociali: hash del source lock diverso.");
  }
  ...
  if (digest(parsed) !== integrity.dataArtifact.sha256 || Buffer.byteLength(serialized) !== integrity.dataArtifact.bytes) {
    throw new Error("Snapshot BesT Relazioni sociali: hash o dimensione dell'artefatto diversi dal lock.");
  }
  const expectedMetadata = {
    schemaVersion: 1,
    datasetId: "istat-bes-relazioni",
  ...
  if (canonical(metadata) !== canonical(expectedMetadata)) {
    throw new Error("Snapshot BesT Relazioni sociali: metadati diversi da fonte, semantica o integrità dichiarate.");
  }
  ...
  for (const row of parsed.observations) {
    ...
    if (!indicator || !territories.has(row.territory) || seen.has(key)
      ...
      throw new Error("Snapshot BesT Relazioni sociali: chiave duplicata o dimensione fuori contratto.");
    }
    seen.add(key);
    if ((row.valueTenths === null) !== (row.status === "n")) {
      throw new Error("Snapshot BesT Relazioni sociali: null e flag di dato non disponibile incoerenti.");
    }
  ...
  if (canonical(nulls) !== canonical(sourceLock.nullCells)) {
    throw new Error("Snapshot BesT Relazioni sociali: identità della cella non disponibile diversa.");
  }
  for (const indicator of parsed.indicators) {
    for (const [slice, count] of Object.entries(indicator.coverage)) {
      if (coverage.get(`${indicator.code}/${slice}`) !== count) {
        throw new Error("Snapshot BesT Relazioni sociali: copertura per indicatore, sesso e anno diversa.");
      }
    }
  }
