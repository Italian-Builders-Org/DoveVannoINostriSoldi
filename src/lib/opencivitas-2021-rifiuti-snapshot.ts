import "server-only";
import snapshotJson from "@/data/generated/opencivitas-2021-rifiuti.json";
import {
  assertOpenCivitas2021RifiutiSnapshot,
  type OpenCivitas2021RifiutiSnapshot,
} from "@/lib/data/opencivitas-2021-rifiuti-contract";
import { resolveOpenCivitasRegionName } from "@/lib/region-query";

export const openCivitas2021RifiutiSnapshot: OpenCivitas2021RifiutiSnapshot =
  assertOpenCivitas2021RifiutiSnapshot(snapshotJson);

export function queryOpenCivitas2021Rifiuti(filters: {
  region?: string;
  code?: string;
  limit?: number;
  offset?: number;
  ...
  if (filters.region === undefined && filters.code === undefined) {
    throw new Error(
      "Specificare regione o codice ISTAT Comune: lo snapshot Rifiuti 2021 completo non viene servito in un'unica risposta.",
    );
  }
  ...
  const matches = openCivitas2021RifiutiSnapshot.municipalities.filter(
  ...
  return {
    datasetId: "opencivitas_rifiuti_2021",
    family: "FC70RIFIUTI",
    function: "RIFIUTI",
    referenceYear: openCivitas2021RifiutiSnapshot.referenceYear,
    publishedAt: openCivitas2021RifiutiSnapshot.publishedAt,
    modifiedAt: openCivitas2021RifiutiSnapshot.modifiedAt,
    pagination: {
    ...
    coverage: openCivitas2021RifiutiSnapshot.coverage,
    methodology: openCivitas2021RifiutiSnapshot.methodology,
    provenance: openCivitas2021RifiutiSnapshot.source,
    caveats: [
      "Snapshot distinto da OpenCivitas FC70TOT 2021 (servizi totali) e da FC80RIFIUTI 2022.",
