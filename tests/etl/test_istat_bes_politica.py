"""BES_06-specific SDMX parsing, coverage and provenance checks, entirely offline."""
import copy
import csv
import io
import json
...
import istat_bes_politica as bes


class BesPoliticaTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        for observation in cls.data["observations"]:
            ...
            row.update(DATA_TYPE=code, REF_AREA=observation["territory"], SEX=observation["sex"],
                       TIME_PERIOD=str(observation["year"]), OBS_STATUS="",
    ...
    def test_domain_units_and_edition_are_locked(self):
        for column, bad in [("UNIT_MEAS", "EURO"), ("DOMAIN", "BES_05"), ("EDITION", "2024"),
                            ("DATAFLOW", "IT1:DF_BES_TERRIT_5(1.0)")]:
    ...
    def test_empty_value_absent_row_and_observed_zero_stay_distinct(self):
        empty = [row for row in self.data["observations"] if row["valueTenths"] is None]
        self.assertEqual(len(empty), 2115)
        self.assertTrue(all(row["status"] is None for row in empty))
        ...
        for raw, flag in [("0", "n"), ("", "n"), ("", "g"), ("12.3", "n")]:
