...
export type OpenCivitas2021RifiutiSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "799b774ba3dbde1d2757a4d5f79b235d920a62bc589f2bd58e5ea7835b9e6514";
...
export function assertOpenCivitas2021RifiutiSnapshot(value: unknown): OpenCivitas2021RifiutiSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC70RIFIUTI: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC70RIFIUTI: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC70RIFIUTI: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC70RIFIUTI: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC70RIFIUTI: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2021RifiutiSnapshot;
}
