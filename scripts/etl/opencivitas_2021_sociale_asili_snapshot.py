#!/usr/bin/env python3
"""Pinned FC70SOCNID 2021 release — Sociale e asili nido, distinct from FC70TOT and from FC80SOCNID 2022."""
...
SPEC_PATH = Path(__file__).with_name("specs") / "opencivitas-2021-sociale-asili.source.json"
SPEC = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
OUTPUT = Path("src/data/generated/opencivitas-2021-sociale-asili.json")
# Filled after the first verified build; --check pins the immutable artifact.
SEMANTIC_SHA256 = "afcce17b6257fbd8d2eb04060fd59f163cff5a326a4990b548c72ad6955c195c"
...
def load_raw_data(payload: bytes) -> dict[str, dict[str, dict[str, str]]]:
    """FC70SOCNID CSV includes Privacy (unlike FC80RIFIUTI)."""
...
def scientific_historical(rows: dict) -> bool:
    return any("E" in rows[code]["value"].upper() for code in MONEY_CODES)
...
def normalize_rows(entities: dict, raw: dict) -> list[dict]:
    ...
    excluded_scientific = set(SPEC["excludedScientificHistorical"])
    ...
    if seen_scientific != excluded_scientific:
        raise StructuralError(
            f"{FAMILY}: Comuni con spesa storica in notazione scientifica diversi dal lock "
            f"(attesi {sorted(excluded_scientific)}, trovati {sorted(seen_scientific)})"
        )
...
def normalize(data: bytes, entities: bytes, indicators: bytes, observed_at: str) -> dict:
    ...
    return {
        "schemaVersion": 1,
        "transformVersion": 1,
        "scope": "ordinary-statute-municipalities-social-nursery-fc70-2021",
        "referenceYear": 2021,
        ...
        "source": {
            ...
            "dataset": "Comuni · Sociale e asili nido · Indicatori e determinanti 2021 (FC70SOCNID)",
        ...
        "methodology": {
            "differenceMeaning": "Differenza tra spesa storica e spesa standard sulla funzione Sociale e asili nido. Non è una misura di spreco.",
            "serviceMeaning": "Il confronto sui servizi usa la media dei Comuni della stessa fascia di popolazione, perimetro Sociale e asili nido.",
            "perCapitaMeaning": "Euro per abitante pubblicati dalla fonte; nessuna popolazione ricostruita o imputata.",
            "coverageWarning": "RSS e Province autonome fuori perimetro; esclusi gli aggregati di Italia, regioni, aree e fasce. Esclusi 9 Comuni con SPESA_STORICA o SPESA_STORICA_PROAB vuoti nella fonte (FST presente). Canistro (066017) resta fuori perché la fonte pubblica la spesa storica in notazione scientifica, non come importo decimale: nessuna imputazione a zero.",
            "rankingWarning": "I livelli della fonte non costituiscono un ranking di efficienza.",
            "functionSeparationWarning": "FC70SOCNID 2021 è distinto da FC70TOT 2021 (servizi totali) e da FC80SOCNID 2022: nessuna somma o confronto silenzioso fra funzioni o annualità.",
        },
    }
