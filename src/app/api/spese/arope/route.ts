...
import { queryEurostatArope } from "@/lib/eurostat-arope-snapshot";

const FILTERS = new Set(["territorio", "anno", "limit", "offset"]);
  ...
  try {
    for (const key of params.keys()) {
      if (!FILTERS.has(key) || params.getAll(key).length !== 1) {
        throw new Error("Usare soltanto territorio, anno, limit e offset, una volta ciascuno.");
      }
    }
    ...
    const result = queryEurostatArope({
