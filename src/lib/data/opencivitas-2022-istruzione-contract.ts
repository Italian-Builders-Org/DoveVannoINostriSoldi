...
export type OpenCivitas2022IstruzioneSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "fa0e14879fe8a5d1519068b49e0ac099841ca4cacbdc0dff5643e6adc5a3db84";
...
export function assertOpenCivitas2022IstruzioneSnapshot(value: unknown): OpenCivitas2022IstruzioneSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC80ISTRUZ: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC80ISTRUZ: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC80ISTRUZ: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC80ISTRUZ: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC80ISTRUZ: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2022IstruzioneSnapshot;
}
