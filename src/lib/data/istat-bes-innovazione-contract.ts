...
import { z } from "zod";
import sourceLock from "../../../scripts/etl/specs/istat-bes-innovazione-2004-2023.source.json";
...
const dataSchema = z.object({
  ...
  datasetId: z.literal("istat-bes-innovazione"),
...
export type IstatBesInnovazioneData = z.infer<typeof dataSchema>;

export function validateIstatBesInnovazioneBundle(data: unknown, metadata: unknown) {
  ...
  if (digest(normalizedLock) !== integrity.sourceLockSha256) {
    throw new Error("Snapshot BesT Innovazione: hash del source lock diverso.");
  }
  ...
  if (digest(parsed) !== integrity.dataArtifact.sha256 || Buffer.byteLength(serialized) !== integrity.dataArtifact.bytes) {
    throw new Error("Snapshot BesT Innovazione: hash o dimensione dell'artefatto diversi dal lock.");
  }
  const expectedMetadata = {
    schemaVersion: 1,
    datasetId: "istat-bes-innovazione",
  ...
  if (canonical(metadata) !== canonical(expectedMetadata)) {
    throw new Error("Snapshot BesT Innovazione: metadati diversi da fonte, semantica o integrità dichiarate.");
  }
  ...
  for (const row of parsed.observations) {
    ...
    if (!indicator || !territories.has(row.territory) || seen.has(key)
      ...
      throw new Error("Snapshot BesT Innovazione: chiave duplicata o dimensione fuori contratto.");
    }
    seen.add(key);
    if ((row.valueTenths === null) !== (row.status === "n" || row.status === "g")) {
      throw new Error("Snapshot BesT Innovazione: null e flag di dato non disponibile incoerenti.");
    }
    if (row.valueTenths !== null && row.valueTenths < 0 && !SIGNED_INDICATORS.has(row.indicator)) {
      throw new Error("Snapshot BesT Innovazione: valore negativo fuori dagli indicatori firmati.");
    }
  ...
  if (canonical(nulls) !== canonical(sourceLock.nullCells)) {
    throw new Error("Snapshot BesT Innovazione: identità della cella non disponibile diversa.");
  }
  for (const indicator of parsed.indicators) {
    for (const [slice, count] of Object.entries(indicator.coverage)) {
      if (coverage.get(`${indicator.code}/${slice}`) !== count) {
        throw new Error("Snapshot BesT Innovazione: copertura per indicatore, sesso e anno diversa.");
      }
    }
  }
