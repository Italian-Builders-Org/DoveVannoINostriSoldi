...
export type OpenCivitas2022ViabilitaSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "c979ff4616dae0ce9a98f83542acd879434bddd12450e71b4e568d086ed10f3d";
...
export function assertOpenCivitas2022ViabilitaSnapshot(value: unknown): OpenCivitas2022ViabilitaSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC80TERRVIAB: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC80TERRVIAB: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC80TERRVIAB: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC80TERRVIAB: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC80TERRVIAB: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2022ViabilitaSnapshot;
}
