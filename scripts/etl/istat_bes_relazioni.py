#!/usr/bin/env python3
"""Build/check the separately locked BesT BES_05 social-relations domain, without network I/O."""
...
SPEC = ROOT / "scripts/etl/specs/istat-bes-relazioni-2011-2024.source.json"
DATA = ROOT / "src/data/generated/istat-bes-relazioni-2011-2024.data.json"
META = ROOT / "src/data/generated/istat-bes-relazioni-2011-2024.meta.json"
DATASET = "istat-bes-relazioni"


class SnapshotError(ValueError):
    """Source bytes or public social-relations contract differ from the reviewed lock."""
...
def load_spec(path: Path = SPEC) -> dict:
    ...
    require(spec["domain"] == {"code": "BES_05", "label": "Relazioni sociali", "edition": "2025"}, "domain differs")
...
def value_tenths(raw: str) -> int:
    try:
        value = Decimal(raw)
        require(value.is_finite() and value >= 0, "invalid social-relations value")
        scaled = value * 10
        require(scaled == scaled.to_integral_value(), "value has more than one decimal")
        return int(scaled)
    except (InvalidOperation, ValueError) as error:
        raise SnapshotError(f"invalid social-relations value: {raw!r}") from error
...
def build_data(payload: bytes, spec: dict) -> dict:
    ...
    for row in reader:
        ...
        code = row["DATA_TYPE"]
        require(code in indicators, "indicator outside social-relations domain")
...
def validate_data(data: dict, spec: dict) -> None:
    ...
    require(len(territories) == 135 and len(indicators) == 2, "duplicate/missing dictionary entry")
    ...
    for row in data["observations"]:
        ...
        if value is None:
            require(row["status"] == "n", "null must preserve unavailable-data flag")
