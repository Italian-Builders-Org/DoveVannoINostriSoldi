...
import { queryOpenCivitas2015 } from "@/lib/opencivitas-2015-snapshot";
...
export function GET(request: NextRequest) {
  ...
  if (year !== null && year !== "2015") {
    return Response.json(
      { error: "Questo endpoint serve solo l'annualità 2015 FC20TOT. Le altre annualità restano nei rispettivi dataset." },
      { status: 400, headers: { "Cache-Control": "no-store" } },
    );
  }
  ...
  try {
    return Response.json(queryOpenCivitas2015({
