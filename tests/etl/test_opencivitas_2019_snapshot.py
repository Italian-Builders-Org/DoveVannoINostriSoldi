...
class FC60ReleaseTests(unittest.TestCase):
    def test_source_sample_reconciles_money_and_service_denominators(self):
        ...
        self.assertEqual(row["historicalSpendingCents"], 308248360020)
        self.assertEqual(row["standardSpendingCents"], 279699907960)
        self.assertEqual(row["historicalPerCapitaCents"], 109764)
        self.assertEqual(row["standardPerCapitaCents"], 99598)
        self.assertEqual(row["differenceBasisPoints"], 1021)
        self.assertEqual(row["serviceDifferenceBasisPoints"], 224)
        self.assertEqual(row["spendingLevel"], 8)
    ...
    def test_metadata_units_function_language_and_duplicates_are_checked(self):
        ...
        for field, value in [("VAR_IND_DES", "Spesa standard - migliaia di euro"),
                             ("VAR_IND_FUNZIONE", "ISTRUZIONE"), ("VAR_IND_LINGUA", "EN")]:
