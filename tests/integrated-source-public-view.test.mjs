...
test("the aggregate release proof closes the fixed public contract", async () => {
  ...
  assert.equal(release.datasets.sourceRows, 14_166_458);
  assert.equal(release.datasets.publicRows, 1_184_112);
...
test("all 90 datasets remain visible and only catalog dispositions decide row access", async () => {
  const overview = await view.getIntegratedDataOverview();
  assert.equal(overview.complete, true);
  assert.equal(overview.datasets.length, 90);
  assert.equal(overview.datasets.filter((dataset) => dataset.queryable).length, 68);
  assert.equal(overview.datasets.reduce((sum, dataset) => sum + dataset.sourceRows, 0), 14_166_458);
  assert.equal(overview.datasets.reduce((sum, dataset) => sum + dataset.publicRows, 0), 1_184_112);
...
test("every queryable artifact passes schema, hash, decompression and URL gates", async () => {
  ...
  assert.equal(checked.length, 68);
  assert.equal(
...
test("enumerating every queryable dataset does not retain all parsed row arrays", async () => {
  ...
  const program = `
    ...
    await (async () => {
      ...
      if (loaded.length !== 68) throw new Error("Unexpected queryable dataset count");
    })();
    await new Promise((resolve) => setImmediate(resolve));
    const after = collect();
    const diagnostics = loader.getIntegratedDatasetLoaderDiagnosticsForTests();
