...
export type OpenCivitas2015Snapshot = {
  schemaVersion: 1;
  transformVersion: 1;
  ...
  source: {
    ...
    family: "FC20TOT"; releaseVersion: 2; releaseVersionReason: string;
...
const SEMANTIC_SHA256 = "bd46ccd14ce5e789f5a9bb0fc657cde6b8256263a56e9e70ce54c250c02abb76";
...
export function assertOpenCivitas2015Snapshot(value: unknown): OpenCivitas2015Snapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC20TOT: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC20TOT: fonte attesa");
  }
  ...
  if (typeof observed !== "string" || observed !== source.observedAt ||
    ...
    throw new Error("FC20TOT: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC20TOT: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (!Array.isArray(record.municipalityColumns) ||
    ...
    throw new Error("FC20TOT: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2015Snapshot;
}
