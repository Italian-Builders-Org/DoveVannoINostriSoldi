...
export type OpenCivitas2021PoliziaSnapshot = {
  schemaVersion: 1;
  transformVersion: 1;
...
const SEMANTIC_SHA256 = "b92132fbc9b811804b9d160efbd7b0cfbd8db5efe769225ec78536efe078d579";
...
export function assertOpenCivitas2021PoliziaSnapshot(value: unknown): OpenCivitas2021PoliziaSnapshot {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("FC70POLIZIA: oggetto snapshot atteso");
  }
  const record = value as Record<string, unknown>;
  const source = record.source as Record<string, unknown> | undefined;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("FC70POLIZIA: fonte attesa");
  }
  ...
  if (
    ...
    throw new Error("FC70POLIZIA: timestamp di acquisizione non valido");
  }
  ...
  if (createHash("sha256").update(canonicalJson(semantic)).digest("hex") !== SEMANTIC_SHA256) {
    throw new Error("FC70POLIZIA: SHA-256 semantico diverso dal rilascio verificato");
  }
  if (
    ...
    throw new Error("FC70POLIZIA: modello comunale non coerente con il rilascio");
  }
  ...
  return { ...structuredClone(metadata), municipalities } as OpenCivitas2021PoliziaSnapshot;
}
