...
export function generateStaticParams() {
  return getEditorialTopics("controlli").map((topic) => ({ topic: topic.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ topic: string }> }): Promise<Metadata> {
  const { topic: slug } = await params;
  const topic = getEditorialTopic("controlli", slug);
  if (!topic) return {};
  return { title: topic.title, description: topic.description };
}

export default async function ControlsTopicRoute({ params }: { params: Promise<{ topic: string }> }) {
  const { topic: slug } = await params;
  const topic = getEditorialTopic("controlli", slug);
  if (!topic) notFound();
