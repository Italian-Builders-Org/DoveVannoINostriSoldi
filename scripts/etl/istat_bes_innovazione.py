#!/usr/bin/env python3
"""Build/check the separately locked BesT BES_11 innovation domain, without network I/O."""
...
SPEC = ROOT / "scripts/etl/specs/istat-bes-innovazione-2004-2023.source.json"
DATA = ROOT / "src/data/generated/istat-bes-innovazione-2004-2023.data.json"
...
class SnapshotError(ValueError):
    """Source bytes or public innovation-domain contract differ from the reviewed lock."""
...
def load_spec(path: Path = SPEC) -> dict:
    ...
    require(
        spec["domain"] == {"code": "BES_11", "label": "Innovazione, ricerca e creatività", "edition": "2025"},
        "domain differs",
    )
...
def value_tenths(raw: str, *, allow_negative: bool) -> int:
    try:
        value = Decimal(raw)
        ...
        raise SnapshotError(f"invalid innovation-domain value: {raw!r}") from error
...
def build_data(payload: bytes, spec: dict) -> dict:
    ...
    for row in reader:
        ...
        code = row["DATA_TYPE"]
        require(code in indicators, "indicator outside innovation domain")
        ...
        observations.append({
            ...
            "valueTenths": None if flag else value_tenths(raw, allow_negative=code in SIGNED_INDICATORS),
            "status": flag or None,
        })
...
def validate_data(data: dict, spec: dict) -> None:
    ...
    for row in data["observations"]:
        ...
        if value is None:
            ...
            require(type(value) is int and row["status"] is None, "invalid value/status")
            if code not in SIGNED_INDICATORS:
                require(value >= 0, "unexpected negative value")
